...
| Streaming von Entscheidungs-Ereignissen in [!DNL Data Lake] und [!DNL Query Service] | The [!DNL Decisioning Service] auto-creates datasets to stream all XDM Decision Events automatically into the [!DNL Data Lake]. The datasets are then available for analysis and reporting using [!DNL Query Service]. |
...
Weitere Informationen finden Sie unter [Decisioning Service – Übersicht](../../decisioning-service/home.md).

## [!DNL Query Service] {#query}

[!DNL Query Service]Mit können Sie in Adobe Experience Platform standardmäßige SQL-Daten zur Abfrage zu verwenden, um viele verschiedene Anwendungsfälle für Analyse und Daten-Management zu unterstützen. It is a serverless tool which allows you to join any datasets in the [!DNL Data Lake] and capture the query results as a new dataset for use in reporting, [!DNL Data Science Workspace], or for ingestion into [!DNL Profile Service].
...
| Konnektivität des Datenbank-Management-Tools | Connect [!DNL Aqua Data Studio] or [!DNL DB Visualizer] to [!DNL Query Service] for data exploration and dataset creation functionality. [!DNL Query Service] unterstützt auch Konnektivität von R Studio. Authentifizierungsinformationen finden Sie auf der Registerkarte „Berechtigungen“. |
