...
Verwenden Sie die Batch-Aufnahme-API, um Daten beliebigen Typs in Adobe Experience Platform aufzunehmen. Die Batch Ingestion API bietet einen sicheren und skalierbaren Mechanismus zur Aufnahme von Daten, die XDM- oder Nicht-XDM-Schemata entsprechen.
...
Weitere Informationen finden Sie in der Produktdokumentation unter [Batch-Aufnahme – Übersicht](../../ingestion/batch-ingestion/overview.md).
...
| Adobe Advertising | DSP-Protokolle aus Advertising sind in [!DNL Experience Platform] verfügbar. |
...
| End-to-End-Monitoring | Überwacht Streaming-Daten von der Aufnahme bis zur Nutzung in [!DNL Data Lake] und [!DNL Real-Time Customer Profile]. |
| Authentifizierte Datenerfassung | Bei Aktivierung der entsprechenden Option wird ein Client-Authentifizierungstoken angefordert. Dies ermöglicht es den Services von Adobe Experience Platform, zwischen Einträgen aus vertrauenswürdigen nicht vertrauenswürdigen Quellen zu unterscheiden. |
...
Weitere Informationen finden Sie in der Produktdokumentation unter [Streaming-Aufnahme – Übersicht](../../ingestion/streaming-ingestion/overview.md).

### Monitoring

Die Batch- und Streaming-Aufnahme von Client- und serverseitigen Geräten nach Adobe Experience Platform wird in Echtzeit überwacht.

| Funktion | Beschreibung |
| -----------| ---------- |
| Monitoring von Streaming-Daten | End-to-End-Überwachung von Streaming-Daten an [!DNL Data Lake], [!DNL Real-Time Customer Profile] und [!DNL Identity Service]. |
...
[!DNL Segmentation Service] definiert eine bestimmte Untergruppe von Profilen aus Ihrem Profilspeicher und beschreibt die Kriterien, die eine vermarktbare Personengruppe in Ihrem Profilspeicher unterscheidet. Segmente können auf Eintragsdaten (z. B. demografischen Informationen) oder Zeitreihen-Ereignissen basieren, die Berührungspunkte von Kunden mit Ihrer Marke darstellen.
...
| Funktion | Beschreibung |
| -----------| ---------- |
| Kennzeichnen [!DNL Experience Platform] Entitäten | Datennutzungs-Labels können auf Ebene von Verbindung, Datensatz und Datensatz-Feld verwaltet und bestimmt werden. Als Grundlage hierfür dienen vordefinierte, kategorisierte Labels. |
| Erstellen von Datennutzungsrichtlinien | Richtlinien für die Datennutzung können auf Basis vorhandener Labels sowie angeforderten Marketing-Aktionen festgelegt und verwaltet werden. |
