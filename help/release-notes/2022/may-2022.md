...
| Funktion | Beschreibung |
| ----------- | ----------- |
| Exportieren der neuesten Profilqualifizierungen [nach täglicher Segmentauswertung](../../destinations/ui/activate-batch-profile-destinations.md#export-full-files) | Sie können jetzt einen Zeitplan für einen vollständigen einmalig oder täglich durchgeführten Dateiexport mit den neuesten Profilqualifizierungen festlegen, nachdem die tägliche Segmentauswertung abgeschlossen ist. |
...
| Globales Schema | [[!UICONTROL Destination]](https://github.com/tumulurik/xdm/blob/master/schemas/destinations/destination.schema.json) | Aufzählungswerte wurden aus `xdm:destinationCategory` entfernt. |
...
| Funktion | Beschreibung |
| --- | --- |
| Integration des Administratorprotokolls im Abfrage-Service | Durch die Integration des Administratorprotokolls im Abfrage-Service werden Einträge zu abfragebezogenen Benutzeraktionen bereitgestellt, die zur Fehlerbehebung oder zur Einhaltung von Unternehmensrichtlinien zur Datenverwaltung und Regulierungsanforderungen verwendet werden können. Vollständige Informationen dazu finden Sie in der [Dokumentation zur Administratorprotokoll-Integration](../../query-service/data-governance/audit-log-guide.md). |
| SQL-Konstrukt ALTER TABLE | Verwenden Sie SQL, um primäre Identitäten in einem Ad-hoc-Datensatz festzulegen. Mit dem Abfrage-Service können Sie Datensatzspalten direkt über SQL mit dem Befehl `ALTER TABLE` entweder als primäre oder sekundäre Identitäten kennzeichnen. |

{style="table-layout:auto"}

Weitere Informationen zu den Funktionen des Abfrage-Service finden Sie in der [Übersicht zum Abfrage-Service](../../query-service/home.md).
...
| Unterstützung für die Aufnahme von Dateien mit `ISO-8859-1`-Codierung | Verwenden Sie den `encoding`, um mithilfe der `ISO-8859-1`-API [!DNL Flow Service] kodierte Dateien mit einer Cloud-Speicherquelle in Experience Platform aufzunehmen. Weitere Informationen finden Sie im Handbuch zum [Erstellen einer Cloud-Speicherplatz-Quellverbindung](../../sources/tutorials/api/collect/cloud-storage.md). |
