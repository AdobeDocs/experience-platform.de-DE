...
| Bulk-GET-Endpunkt | Der Massenendpunkt GET wurde [!DNL Real-Time Customer Profile] API hinzugefügt, um das Abrufen mehrerer Ergebnisse in einem einzelnen API-Aufruf zu ermöglichen. Sie können jetzt GET mit bis zu 100 IDs für Segmentdefinitionen, Segmentaufträge und Zusammenführungsrichtlinien stapeln. |
| Profil nach Identität durchsuchen | In der [!DNL Platform] Benutzeroberfläche können Sie jetzt einen Identity-Namespace auswählen und einen Identitätswert angeben, um ein Profil zu durchsuchen. |
