...
In den folgenden Beispielen wurde eine CSV-Beispielausgabe zur Veranschaulichung verwendet. Im Folgenden finden Sie einige Merkmale der Beispieldatei.

- Die Datei enthielt keine tokenisierten Ereignisse.
- Die Datei enthielt nicht nur Ereignisse zur Konvertierung (sie enthielt keine Ergebniszeilen mit 0 als marginales Ergebnis).
- Datenmerkmale:
   - 368 Musterzeilen insgesamt.
   - Mindestens 8 Konvertierungen mit je 3 Kanälen.
   - 151 Konvertierungen des Konvertierungstyps `“Digital_Product_Purchase”`.
   - 10 verschiedene Touchpoints, EMAIL, SOCIAL_LINKEDIN, ADS_GOOGLE, SOCIAL_OTHER, ADS_OTHER, SOCIAL_TWITTER , LANDINGPAGE, SOCIAL_FB, ADS_BING, DRUCKEN.
   - Konversionen und Touchpoints liegen zwischen 8 und 9 Monaten.
   - Zeilen werden sortiert nach `id`, `conversion_timestamp` und `touchpoint_timestamp`.
