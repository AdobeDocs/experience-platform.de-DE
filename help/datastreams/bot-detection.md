...
Verwenden Sie die Bot-Erkennung, um von der [Web SDK](/help/collection/js/js-overview.md), [Mobile SDK](https://developer.adobe.com/client-sdks/home/) und [Edge Network API](https://developer.adobe.com/data-collection-apis/docs/api/) generierte Ereignisse als von bekannten Spiders und Bots generiert zu identifizieren.
...
Wenn eine Anfrage an die [!DNL Edge Network] mit einer der Regeln zur Bot-Erkennung übereinstimmt, wird das XDM-Schema mit einem Bot-Score aktualisiert (immer auf 1 gesetzt):
...
>[!IMPORTANT]
>
>Die Bot-Erkennung löscht keine Bot-Anfragen. Es wird nur das XDM-Schema mit der Bot-Bewertung aktualisiert und das Ereignis an den [Datenstrom-Service](/help/datastreams/configure.md) weitergeleitet.
>
>Adobe-Lösungen können die Bewertung von Bots auf unterschiedliche Weise verarbeiten. Beispielsweise verwendet [!DNL Adobe Analytics] einen eigenen [Bot-Filterdienst](https://experienceleague.adobe.com/docs/analytics/admin/admin-tools/manage-report-suites/edit-report-suite/report-suite-general/bot-removal/bot-rules.html) und verwendet nicht den vom [!DNL Edge Network] festgelegten Wert. Die beiden Services verwenden dieselbe [IAB-Bot-Liste](https://www.iab.com/guidelines/iab-abc-international-spiders-bots-list/) sodass die Bot-Bewertung identisch ist.
...
* Es kann bis zu 15 Minuten dauern, bis sich Bot-Erkennungsregeln nach ihrer Erstellung über die [!DNL Edge Network] ausbreiten.
...
Sie können die Bot-Erkennung konfigurieren, nachdem Sie eine Datenstromkonfiguration erstellt haben. Weitere Informationen finden Sie in der Dokumentation [Erstellen und Konfigurieren eines Datenstroms](/help/datastreams/configure.md) und befolgen Sie dann die folgenden Anweisungen, um Ihrem Datenstrom Funktionen zur Bot-Erkennung hinzuzufügen.
...
3. Wählen Sie **[!UICONTROL Add new IP condition]** aus, um eine neue IP-basierte Regel hinzuzufügen. Sie können die Regel nach IP-Adresse oder IP-Adressbereich definieren.
   ...
   >[!TIP]
   >
   >Die IP-Bedingungen basieren auf einem logischen [!DNL OR]. Eine Anfrage wird als von einem Bot stammend markiert, wenn sie einer der von Ihnen definierten IP-Bedingungen entspricht.
...
4. Wenn Sie Ihrer Regel Kopfzeilenbedingungen hinzufügen möchten, wählen Sie **[!UICONTROL Add header conditions group]** und dann die Kopfzeilen aus, die die Regel verwenden soll.
   ...
   ![Bildschirm für Bot-Erkennungsregeln mit den ausgefüllten Header-Bedingungsfeldern.](assets/bot-detection/header-condition-rule.png)
...
5. Wählen Sie nach dem Konfigurieren der gewünschten Regeln zur Bot-Erkennung die Option **[!UICONTROL Save]** aus, damit die Regeln auf Ihren Datenstrom angewendet werden.
   ...
   ![Bildschirm für Bot-Erkennungsregeln mit hervorgehobener Schaltfläche „Speichern“.](assets/bot-detection/bot-detection-save.png)
...
Um alle Anfragen, die von einer bestimmten IP-Adresse stammen, als Bot-Traffic zu markieren, erstellen Sie eine neue Regel zur Bot-Erkennung, die eine einzelne IP-Adresse auswertet.

![Bot-Erkennungsregel konfiguriert, um eine einzelne IP-Adresse zu bewerten.](assets/bot-detection/bot-detection-one-ip.png)

### Bot-Erkennung basierend auf zwei IP-Adressen {#two-ip}

Um alle Anfragen, die von einer von zwei bestimmten IP-Adressen stammen, als Bot-Traffic zu markieren, erstellen Sie eine neue Regel zur Bot-Erkennung, die zwei IP-Adressen auswertet.

![Bot-Erkennungsregel konfiguriert, um zwei bestimmte IP-Adressen auszuwerten.](assets/bot-detection/bot-detection-two-ips.png)

### Bot-Erkennung basierend auf einem Bereich von IP-Adressen {#range}

Um alle Anfragen, die von einer IP-Adresse in einem bestimmten Bereich stammen, als Bot-Traffic zu markieren, erstellen Sie eine neue Regel zur Bot-Erkennung, die einen gesamten IP-Adressbereich auswertet.

![Bot-Erkennungsregel konfiguriert, um einen IP-Adressbereich zu bewerten.](assets/bot-detection/bot-detection-range.png)

### Bot-Erkennung basierend auf einer IP-Adresse und einem Anfrage-Header {#ip-header}

Um alle Anfragen, die von einer bestimmten IP-Adresse stammen und einen bestimmten Anfrage-Header enthalten, als Bot-Traffic zu markieren, erstellen Sie eine neue Regel zur Bot-Erkennung. Diese Regel prüft, ob die Anfrage von einer bestimmten IP-Adresse stammt und ob der `referer`-Header mit `www.adobe.com` beginnt.

![Bot-Erkennungsregel konfiguriert, um eine IP-Adresse und den Referrer-Anfrage-Header zu bewerten.](assets/bot-detection/bot-detection-header-ip.png)
...
* **Mehrere verschiedene Bedingungen**: Verschiedene Bedingungen werden als logischer [!DNL AND]-Vorgang ausgewertet, d. h., alle Bedingungen müssen gleichzeitig erfüllt sein, damit das System die Anfrage als Bot-Traffic identifizieren kann.
* **Mehrere Bedingungen desselben Typs**: Bedingungen desselben Typs werden als logischer [!DNL OR]-Vorgang ausgewertet, d. h. wenn eine der Bedingungen erfüllt ist, wird die Anfrage als Bot-Traffic identifiziert.

Die folgende Regel identifiziert eine Bot-Ursprungsanfrage, wenn diese Bedingungen erfüllt sind: Die Anfrage stammt von einer der beiden IP-Adressen, der `referer`-Header beginnt mit `www.adobe.com` und der `sec-ch-ua-mobile`-Header identifiziert die Anfrage als von einem Desktop-Browser stammend.

![Bot-Erkennungsregel mit mehreren IP-, Referer- und Benutzeragenten-Bedingungen konfiguriert.](assets/bot-detection/bot-detection-multiple.png)
