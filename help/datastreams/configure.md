...
Wenn Sie eines der oben genannten Felder für die Datenerfassung aktivieren, stellen Sie sicher, dass Sie beim Konfigurieren der Web-SDK die [`context`](/help/collection/js/commands/configure/context.md)-Array-Eigenschaft richtig festgelegt haben.
...
| [!UICONTROL First Party ID Cookie] | Wenn diese Einstellung aktiviert ist, weist sie das Edge-Netzwerk an, bei der Suche nach einer [First-Party-Geräte-ID](/help/collection/use-cases/identity/first-party-device-ids.md) ein bestimmtes Cookie zu verwenden, anstatt nach diesem Wert in der Identity Map zu suchen.<br><br>Wenn Sie diese Einstellung aktivieren, müssen Sie den Namen des Cookies angeben, in dem die ID gespeichert werden soll. |
...
| [!UICONTROL Third Party ID Sync Container ID] | Die numerische ID des Containers, der für die ID-Synchronisierung von Drittanbietern verwendet werden soll.<br><br>**Hinweis:** Der Datenstrom verweist auf die standardmäßige Audience Manager-Container-ID, nämlich 0. Wenn Sie mehrere IDs für den Audience Manager ID-Synchronisierungs-Container haben, wenden Sie sich an Ihren Audience Manager-Berater, um die ID-Synchronisierungskonfiguration ordnungsgemäß zu identifizieren und zu beheben. |
...
>[!NOTE]
>
>Warten Sie nach dem Speichern der Änderungen an einer Datenstromkonfiguration bis zu 35 Minuten, damit die Änderungen in Edge Network übernommen werden. Während dieses Übertragungsfensters können Anfragen weiterhin mit der vorherigen Konfiguration bedient werden.
...
Mit diesem Service wird festgelegt, ob und wie Daten an Adobe Analytics gesendet werden.
...
| [!UICONTROL Visitor ID namespace] | (Optional) Der Namespace, den Sie für die Adobe Analytics verwenden möchten [visitorID](https://experienceleague.adobe.com/docs/analytics/implementation/vars/config-vars/visitorid.html?lang=de). Wenn Sie ein Ereignis senden, dessen Wert für diesen Namespace angegeben ist, wird es automatisch als `visitorID` in Analytics verwendet. <br> Wenn das Feld ausgefüllt ist, sendet der Datenstrom den `visitorID` an Adobe Analytics. Unabhängig davon, ob die Besucher-ID im `identityMap` enthalten ist, wird die `ECID` dennoch generiert und in ausgehenden Anfragen enthalten. Analytics unterstützt die Einbeziehung mehrerer IDs. Die IDs werden in der auf dieser Seite beschriebenen Reihenfolge ausgewertet: [Adobe Analytics-Identifizierungsreihenfolge der Vorgänge](https://experienceleague.adobe.com/en/docs/analytics/implementation/id/overview#adobe-analytics-identification-order-of-operations). |
...
Weitere Informationen [&#x200B; Sie im Analytics](https://experienceleague.adobe.com/en/docs/analytics/implementation/aep-edge/overview)Implementierungshandbuch unter „Implementieren von Adobe Analytics mit Edge Network&quot;.
...
In diesem Handbuch wurde beschrieben, wie Datenströme in der Datenerfassungs-Benutzeroberfläche verwaltet werden. Weitere Informationen zum Installieren und Konfigurieren von Web SDK nach dem Einrichten eines Datenstroms finden Sie unter [Erste Schritte mit der Tag-Erweiterung für Web SDK](../tags/extensions/client/web-sdk/getting-started.md).
