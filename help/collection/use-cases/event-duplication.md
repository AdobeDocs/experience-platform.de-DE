...
* Der Real-Time CDP-Profilspeicher löscht Ereignisse, wenn im `_id` bereits ein Ereignis mit demselben [!DNL Profile store] vorhanden ist. Weitere Informationen finden Sie in der Dokumentation [XDM ExperienceEvent](/help/xdm/classes/experienceevent.md)Klasse).
...
* Experience Platform Query Service unterstützt die Datendeduplizierung, wenn eine ganze Zeile aus einer Berechnung entfernt oder ein bestimmter Feldsatz ignoriert werden muss, da nur ein Teil der Daten in der Zeile doppelte Informationen enthält. Weitere Informationen finden Sie in der Dokumentation [Datendeduplizierung im Abfrage](/help/query-service/key-concepts/deduplication.md)Service).
