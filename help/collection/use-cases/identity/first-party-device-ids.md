...
>[!IMPORTANT]
>
>IDs von Erstanbieter-Geräten sind nicht mit der Funktion [Drittanbieter-Cookies](/help/tags/extensions/client/web-sdk/configure/identity.md) in Web SDK kompatibel. Sie können entweder Erstanbieter-Geräte-IDs oder Drittanbieter-Cookies verwenden, aber nicht beide gleichzeitig.

## Voraussetzungen {#prerequisites}

Bevor Sie beginnen, stellen Sie sicher, dass Sie mit der Funktionsweise von Identitätsdaten in der Web-SDK, einschließlich ECIDs und `identityMap`, vertraut sind. Weitere Informationen finden Sie in der Übersicht [Identitätsdaten in der Web](id-overview.md)SDK) .
...
Auf Cookies, die mit dem `HTTPOnly`-Flag gesetzt werden, kann nicht über Client-seitige Skripte zugegriffen werden. Wenn Sie also beim Festlegen der `HTTPOnly` ein [!DNL FPID]-Flag festlegen, müssen Sie eine Server-seitige Skriptsprache verwenden, um den Cookie-Wert zur Aufnahme in die `identityMap` zu lesen.
...
1. In der [!DNL ECID] enthaltene `identityMap`
1. In einem Cookie gespeicherte [!DNL ECID]
1. In der [!DNL FPID] enthaltene `identityMap`
...
>
>Das Adobe-Managed Certificate Program[&#x200B; von &#x200B;](https://experienceleague.adobe.com/docs/core-services/interface/administration/ec-cookies/cookies-first-party.html?lang=de#adobe-managed-certificate-program) wird auch für die Erstanbieter-Datenerfassung unterstützt.
...
Nehmen wir an, Sie möchten Web SDK auf Ihrer Website-`example.com` implementieren. Web SDK sendet Daten an die Edge Network an die `edge.adobedc.net` Domain.

| Ohne [!DNL CNAME] | Mit [!DNL CNAME] |
|---------|----------|
| <ul><li>Ihre Website verwendet `example.com` die Domain-`edge.adobedc.net` Web SDK , um Daten an die Edge Network zu senden.</li><li>Von `edge.adobedc.net` gesetzte Cookies gelten als Drittanbieter-Cookies, da sie nicht von Ihrer `example.com` Domain stammen. Abhängig von Ihren Benutzerbrowsern werden Drittanbieter-Cookies möglicherweise blockiert, und Ihre Daten erreichen die Edge Network nicht.</li></ul> | <ul><li>Sie erstellen eine Subdomain, in der Sie Web-SDK bereitstellen, z. B. `metrics.example.com`.</li><li>Sie legen in Ihrem DNS-System einen [!DNL CNAME]-Eintrag fest, sodass `metrics.example.com` auf `edge.adobedc.net` verweist.</li><li>Wenn Ihre Website Cookies über `metrics.example.com` setzt, scheinen diese beim Browser von `example.com` (Erstanbieter) und nicht von `edge.adobedc.net` (Drittanbieter) zu stammen. Dadurch sinkt die Wahrscheinlichkeit, dass das Erstanbieter-ID-Cookie blockiert wird, was eine genauere Datenerfassung gewährleistet.</li></ul> |
