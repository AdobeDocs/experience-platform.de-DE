...
Wählen Sie **[!UICONTROL Extensions]** in der linken Navigationsleiste aus. Wählen Sie auf der Registerkarte **[!UICONTROL Catalog]** die Option **[!UICONTROL Install]** auf der Karte für die [!DNL Twitter] aus.
...
Erstellen Sie eine neue [Regel](../../../ui/managing-resources/rules.md) in Ihrer Ereignisweiterleitungs-Eigenschaft. Fügen Sie unter **[!UICONTROL Actions]** eine neue Aktion hinzu und legen Sie die Erweiterung auf **[!UICONTROL Twitter]** fest. Um Edge Network-Ereignisse an [!DNL Twitter] zu senden, setzen Sie den **[!UICONTROL Action Type]** auf **[!UICONTROL Send Web Conversion].**
...
| --- | --- | --- | --- |
| [!UICONTROL [!DNL Twitter] Click ID] | [!DNL Twitter] Klicken auf die ID, die von der Clickthrough-URL geparst wurde. | `26l6412g5p4iyj65a2oic2ayg2` | Erforderlich, wenn keine andere Kennung hinzugefügt wird. |
| [!UICONTROL Email] | Eine mit SHA256 gehashte E-Mail-Adresse. Der Text muss in Kleinbuchstaben geschrieben werden, und alle nachfolgenden oder führenden Leerzeichen müssen vor dem Hashing entfernt werden. | `eventforwarding@example.com` | Erforderlich, wenn keine andere Kennung hinzugefügt wird. |
| [!UICONTROL Phone] | Telefon dient als Kennung für das Konversionsereignis. Die Telefonnummer muss vor dem Hashing im E164-Format `[+][country code][area code][local phone number]` sein. | `+911234567875` | Erforderlich, wenn keine andere Kennung hinzugefügt wird. |

**[!UICONTROL Conversion Data]**

| Feldname | Beschreibung | Beispiel | Erforderlich |
| --- | --- | --- | --- |
| [!UICONTROL Conversion Time] | Datum-Uhrzeit als Zeichenfolge in ISO 8601 oder im `yyyy-MM-dd'T'HH:mm:ss:SSSZ`. | 2022-02-18T01:14:00.603Z | Ja |
| [!UICONTROL Event Id] | Die Base-36-ID eines bestimmten Ereignisses. Diese ID sollte mit einem vorkonfigurierten Ereignis übereinstimmen, das in Ihrem [!DNL Twitter]-Werbekonto enthalten ist. Dies wird als ID für das entsprechende Ereignis im Ereignis-Manager bezeichnet. | o87ne oder tw-o8z6j-o87ne (tw-pixel_id-event-id) | Ja |
| [!UICONTROL Number of Items] | Die Anzahl der Artikel, die im Ereignis gekauft werden. Dies muss eine positive Zahl größer als 0 sein. | 4 | Nein |
| [!UICONTROL Currency] | Die Währung der Artikel, die im Ereignis gekauft werden. Dies wird in ISO-4217 ausgedrückt. Wenn dies nicht angegeben wird, lautet der Standardwert USD. | USD | Nein |
| [!UICONTROL Value] | Der Preiswert der Artikel, die im Ereignis gekauft werden. | 100,00 | Nein |
| [!UICONTROL Conversion ID] | Eine Kennung für ein Konversionsereignis, die für die Deduplizierung zwischen Web-Pixel- und Konversions-API-Konversionen im selben Ereignis-Tag verwendet werden kann. | 23294827 | Nein |
| [!UICONTROL Description] | Eine Beschreibung mit zusätzlichen Informationen zu den Konversionen. | Konvertierung testen | Nein |
...
Wenn die Ereignissammlung und [!DNL Experience Platform] Integration erfolgreich waren, werden Ereignisse im [!DNL Twitter] [!UICONTROL Events manager] angezeigt.
