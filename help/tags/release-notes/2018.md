...
description: Versionshinweise für Tags in Adobe Experience Platform 2018.
...
* **Fehlerkorrektur** - Es wurde ein Problem behoben, bei dem eine Regel, die ein Seitenende-Ereignis und eine Aktion mit benutzerspezifischem Code auf einer Seite verwendet, auf der Tags zwar synchron geladen, aber nicht ordnungsgemäß installiert wurden (kein Aufruf von  `_satellite.pageBottom()`), den Website-Inhalt gelöscht hat.
* **Fehlerkorrektur** - Es wurde ein Problem behoben, bei dem der Viewport-Aufruf nicht funktionierte, wenn die Tag-Bibliothek asynchron geladen wurde und das Laden abgeschlossen wurde, nachdem das DOMContentLoaded-Ereignis des Browsers ausgelöst wurde.
...
* Es wurde ein Problem behoben, bei dem einige Schaltflächen in Tag-Erweiterungen nicht den richtigen Abstand hatten.
...
Wird von Tausenden von Kunden für das Web-Tag-Management verwendet, kann der Tag-Arbeitsbereich jetzt zur Verwaltung Ihrer mobilen SDKs verwendet werden.

Durch das Erstellen einer mobilen Eigenschaft in der Datenerfassungs-Benutzeroberfläche haben Sie folgende Möglichkeiten:
...
* Verwalten mobiler Erweiterungen:
   ...
   * Erweiterungen sind für andere Adobe-Lösungen verfügbar, darunter [!DNL Adobe Analytics], [!DNL Adobe Target] und [!DNL Adobe Audience Manager].
* Die Mobile Core-Erweiterung und alle zusätzlichen Erweiterungen können über Abhängigkeitsmanager wie Maven und CocoaPods heruntergeladen und installiert werden.
...
Neu erstellte Tag-Eigenschaften enthalten jetzt standardmäßig einen [!DNL "Managed by Adobe"] -Host und drei Umgebungen (eine von jedem Typ).
...
Adobe Experience Platform-Benutzer werden jetzt benachrichtigt, wenn neue Versionen von Erweiterungen verfügbar sind und sie selbst installieren können. Weitere Informationen finden Sie [hier](../ui/managing-resources/extensions/extension-upgrade.md).
...
Die Tags und [!DNL DTM]-Bibliotheken mit [!DNL Akamai] sind jetzt auf CDN-Edge-Knoten in China verfügbar. Dies bietet deutlich schnellere Bibliotheksladegeschwindigkeiten für Endbenutzer in China.
...
Die Open-Source-Dokumentation für Tags finden Sie unter [Experience League](https://experienceleague.adobe.com/docs/launch/using/overview.html?lang=de).
...
Die Regel-ID wird jetzt für jede Regel in einem Build ausgegeben und kann im Browser referenziert werden.
...
* Formularfelder haben jetzt den Standardfokus
...
#### Verknüpfen des eingebetteten DTM-Codes mit Tags

Wenn Sie Ihren [!DNL DTM]-Einbettungscode mit Tags verknüpfen, können Sie den [!DNL DTM] -Einbettungscode für die Produktion auf einer Seite belassen, dort jedoch Tag-Dateien anstelle von [!DNL DTM] bereitstellen.
...
* Erstellen einer neuen Bibliothek aus dem Dropdown-Menü &quot;Aktive Bibliothek&quot;
...
Cache-Control-Header werden jetzt automatisch für Bibliotheken festgelegt, die auf [!DNL Akamai] (assets.adobedtm.com) gehostet werden. Zuvor wurden keine Cache-Control-Header für Dateien festgelegt, die auf assets.adobedtm.com gehostet wurden.

* Produktions-Builds: Cache-Control-Header sind auf 60 Minuten eingestellt
* Staging-Builds mit &quot;-staging&quot;im Dateinamen: Cache-Control-Header werden auf 0 Minuten eingestellt
* Entwicklungs-Builds mit &quot;-development&quot;im Dateinamen: Cache-Control-Header werden auf 0 Minuten eingestellt
...
* Es wurde ein Problem behoben, bei dem die Tag-Bibliothek in bestimmten Edge-Fällen zweimal geladen wurde.
...
Tags ist die nächste Generation von Tag-Management-Funktionen, die von Adobe erstellt werden. Dadurch erhalten Kunden eine einfache Möglichkeit, alle Analyse-, Marketing- und Werbe-Tags bereitzustellen und zu verwalten, die für relevante Kundenerlebnisse erforderlich sind.

Mit Tags kann jeder mithilfe von Erweiterungen eigene Integrationen erstellen und verwalten. Diese Erweiterungen stehen Platform-Kunden in einer App Store-Oberfläche zur Verfügung, damit sie ihre Tags schnell installieren, konfigurieren und bereitstellen können.

Tags werden Adobe Experience Cloud-Kunden als integrierte Funktion zum Hinzufügen von Mehrwert bereitgestellt. Tags ist ein völlig neues Produkt mit einer neuen Codebasis, das den vorherigen [!DNL Dynamic Tag Management (DTM)]-Dienst ersetzen soll. [!DNL DTM] wird jedoch auf absehbare Zeit weiterhin unterstützt. [!DNL Adobe] wird weiterhin bedeutende Fehler beheben und eine konsistente Leistung sicherstellen. Momentan sind keine grundlegenden Funktionsverbesserungen für die alte Version von [!DNL DTM] geplant.
...
Eine Erweiterung ist ein Paket mit Code (JavaScript, HTML und CSS), das die Datenerfassungs-Benutzeroberfläche und die Client-Funktionalität erweitert. &#x200B;Erstellen, verwalten und aktualisieren Sie Ihre Integrationen mithilfe einer praktischen Self-Service-Oberfläche. Sie können sich ein Tag als Betriebssystem vorstellen, und Erweiterungen sind die Apps, mit denen Sie Ihre Aufgaben erledigen.
...
* Tags interagieren mit den [!DNL Reactor]-APIs &#x200B;
* Implementierungen können über APIs automatisiert werden &#x200B;
* Integrieren Sie die Tags-APIs in Ihre eigenen internen Systeme &#x200B;
* Sie können bei Bedarf Ihre eigene Benutzeroberfläche erstellen &#x200B;

#### Einfaches, modulares Container-Tag

Das von Tags erstellte Container-Tag ist um 60 % heller als [!DNL Adobe Tag Manager] und um 40 % heller als [!DNL Google Tag Manager]. Der Inhalt Ihres Containers wird minimiert, einschließlich Ihres benutzerspezifischen Codes. Alles ist modular. Wenn Sie ein Element nicht benötigen, ist es nicht in Ihrer Bibliothek enthalten. Das Ergebnis ist eine schnelle und kompakte Implementierung.
