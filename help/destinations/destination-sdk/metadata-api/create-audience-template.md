...
Sie können eine neue Zielgruppenvorlage erstellen, indem Sie eine `POST` Anfrage an `/authoring/audience-templates` -Endpunkt.
...
Nach dem Lesen dieses Dokuments wissen Sie jetzt, wann Zielgruppenvorlagen verwendet und wie eine Zielgruppenvorlage mithilfe der `/authoring/audience-templates` API-Endpunkt. Lesen Sie [Verwenden des Destination SDK zum Konfigurieren Ihres Ziels](../guides/configure-destination-instructions.md), um zu verstehen, wo dieser Schritt in den Prozess der Konfiguration Ihres Ziels passt.
