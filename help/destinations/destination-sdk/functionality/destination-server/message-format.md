...
Weitere Informationen zu Identitäten in Experience Platform finden Sie in der [Übersicht über Identity-Namespaces](../../../../identity-service/features/namespaces.md).
...
| `hasSegments` | Diese Funktion akzeptiert eine Zuordnung von Namespace-Zielgruppen-IDs als Parameter. Die Funktion gibt `true` zurück, wenn mindestens eine Zielgruppe in der Zuordnung vorhanden ist (unabhängig vom Status), andernfalls `false`. Sie können diese Funktion verwenden, um zu entscheiden, ob eine Zielgruppenzuordnung durchlaufen werden soll oder nicht. | `hasSegments(input.profile.segmentMembership)` |
...
| `destination.namespaceSegmentTimestamps` | Gibt den Zeitpunkt zurück, zu dem eine Zielgruppe im UNIX-Zeitstempelformat erstellt, aktualisiert oder aktiviert wurde. | <ul><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].createdAt`: gibt den Zeitpunkt zurück, zu dem das Segment mit der ID `seg-id-1` aus dem Namespace `ups` im UNIX-Zeitstempelformat erstellt wurde.</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].updatedAt`: gibt den Zeitpunkt zurück, zu dem die Audience mit der ID `seg-id-1` aus dem Namespace `ups` im UNIX-Zeitstempelformat aktualisiert wurde.</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].mappingCreatedAt`: gibt die Zeit zurück, zu der die Audience mit der ID `seg-id-1` aus dem Namespace `ups` für das Ziel aktiviert wurde, im UNIX-Zeitstempelformat.</li><li>`destination.namespaceSegmentTimestamps["ups"]["seg-id-1"].mappingUpdatedAt`: gibt den Zeitpunkt zurück, zu dem die Zielgruppenaktivierung am Ziel im UNIX-Zeitstempelformat aktualisiert wurde.</li></ul> |
| `addedSegments(mapOfNamespacedSegmentIds)` | Gibt nur die Zielgruppen mit dem Status &quot;`realized`&quot;für alle Namespaces zurück. | `addedSegments(input.profile.segmentMembership)` |
| `removedSegments(mapOfNamespacedSegmentIds)` | Gibt nur die Zielgruppen mit dem Status &quot;`exited`&quot;für alle Namespaces zurück. | `removedSegments(input.profile.segmentMembership)` |
| `destination.segmentAliases` | **Veraltet. Ersetzt durch`destination.namespaceSegmentAliases`** <br><br> Zuordnung von Zielgruppen-IDs im Adobe Experience Platform-Namespace zu Zielgruppen-Aliasen im System des Partners. | `destination.segmentAliases["seg-id-1"]` |
...
| `destination.segmentTimestamps` | **Veraltet. Ersetzt durch`destination.namespaceSegmentTimestamps`** <br><br> Gibt den Zeitpunkt zurück, zu dem eine Zielgruppe im UNIX-Zeitstempelformat erstellt, aktualisiert oder aktiviert wurde. | <ul><li>`destination.segmentTimestamps["seg-id-1"].createdAt`: gibt den Zeitpunkt zurück, zu dem die Zielgruppe mit der ID `seg-id-1` im UNIX-Zeitstempelformat erstellt wurde.</li><li>`destination.segmentTimestamps["seg-id-1"].updatedAt`: gibt den Zeitpunkt zurück, zu dem die Zielgruppe mit der ID `seg-id-1` im UNIX-Zeitstempelformat aktualisiert wurde.</li><li>`destination.segmentTimestamps["seg-id-1"].mappingCreatedAt`: gibt den Zeitpunkt zurück, zu dem die Zielgruppe mit der ID `seg-id-1` im UNIX-Zeitstempelformat für das Ziel aktiviert wurde.</li><li>`destination.segmentTimestamps["seg-id-1"].mappingUpdatedAt`: gibt den Zeitpunkt zurück, zu dem die Zielgruppenaktivierung am Ziel im UNIX-Zeitstempelformat aktualisiert wurde.</li></ul> |
