...
Sie können die in diesem Dokument beschriebene Funktion mithilfe des API-Endpunkts `/authoring/audience-templates` konfigurieren. Eine vollständige Liste der Vorgänge, die Sie für den Endpunkt ausführen können, finden Sie unter [API-Vorgänge für Zielmetadaten-Endpunkte](./audience-metadata-api.md) .
...
Dank der Unterstützung von Zielgruppen-Metadaten im Ziel-SDK können Sie Plattformbenutzern beim Konfigurieren Ihres Experience Platform-Ziels eine von mehreren Optionen geben, wenn sie  Ihrem Ziel zuordnen und aktivieren. Sie können die dem Benutzer zur Verfügung stehenden Optionen über die Parameter im Abschnitt Segmentzuordnung der [Zielkonfiguration](./destination-configuration.md#segment-mapping) steuern.

### Nutzungsszenario 1 - Sie verfügen über eine Drittanbieter-API und Benutzer müssen keine Zuordnungs-IDs eingeben

Wenn Sie über einen API-Endpunkt zum Erstellen/Aktualisieren/Löschen von Segmenten oder Zielgruppen verfügen, können Sie Zielgruppen-Metadatenvorlagen verwenden, um das Ziel-SDK so zu konfigurieren, dass es den Spezifikationen Ihres Segmentendpunkts zum Erstellen/Aktualisieren/Löschen entspricht. Experience Platform kann Segmente programmgesteuert erstellen/aktualisieren/löschen und Metadaten wieder mit Experience Platform synchronisieren.
...
Sie können die generische Vorlage verwenden, um [eine neue Zielgruppenvorlage](./audience-metadata-api.md#create) zu erstellen, wenn Ihre API Folgendes unterstützt:
...
Beachten Sie, dass in einigen Beispielen Makro-Felder wie `{{authData.accessToken}}` oder `{{segment.name}}` in der URL verwendet werden und in anderen Beispielen in den Kopfzeilen oder im Anfrageinhalt verwendet werden. Das hängt wirklich von Ihren Marketing-API-Spezifikationen ab.
...
| `validations` | Führt Überprüfungen für alle Felder in der Vorlagenkonfiguration durch, bevor Sie die Partner-API aufrufen. Sie können beispielsweise überprüfen, ob die Konto-ID des Benutzers korrekt eingegeben wurde. |

{style=&quot;table-layout:auto&quot;}

### Erstes Beispiel {#example-1}
...
Beschreibungen aller Parameter in der Vorlage finden Sie in der Referenzdokumentation [API-Vorgänge für Zielmetadaten-Endpunkte](./audience-metadata-api.md).
...
| `{{authData.accessToken}}` | Ermöglicht die Weitergabe des Zugriffs-Tokens an Ihren API-Endpunkt. Verwenden Sie `{{authData.accessToken}}`, wenn Experience Platform nicht ablaufende Token für die Verbindung mit Ihrem Ziel verwenden soll. Verwenden Sie andernfalls `{{oauth2ServiceAccessToken}}`, um ein Zugriffstoken zu generieren. |
| `{{body.segments[0].segment.id}}` | Gibt die eindeutige Kennung der erstellten Audience als Wert des Schlüssels `externalAudienceId` zurück. |
