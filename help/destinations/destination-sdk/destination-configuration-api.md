...
| `customerAuthenticationConfigurations.authType` | Zeichenfolge | Folgende Werte werden für Streaming-Ziele unterstützt: <ul><li>`BASIC`</li><li>`BEARER`</li><li>`OAUTH2`</li></ul> Folgende Werte werden für dateibasierte Ziele unterstützt: <ul><li>`S3`</li><li>`AZURE_CONNECTION_STRING`</li><li>`AZURE_SERVICE_PRINCIPAL`</li><li>`SFTP_WITH_SSH_KEY`</li><li>`SFTP_WITH_PASSWORD`</li></ul> |
