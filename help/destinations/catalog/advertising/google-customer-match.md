...
description: Google Customer Match verwendet Ihre Online- und Offline-Daten, um Ihre Kunden in den von Google verwalteten und betriebenen Eigenschaften wie Search, Shopping und Gmail zu erreichen und erneut mit ihnen zu interagieren.
...
>[!IMPORTANT]
>
> Google veröffentlicht Änderungen an der [Google Ads API](https://developers.google.com/google-ads/api/docs/start), [Customer Match](https://ads-developers.googleblog.com/2023/10/updates-to-customer-match-conversion.html) und der [Display &amp; Video 360 API](https://developers.google.com/display-video/api/guides/getting-started/overview), um die Compliance- und Zustimmungsanforderungen zu unterstützen, die im [Digital Markets Act](https://digital-markets-act.ec.europa.eu/index_en) (DMA) in der Europäischen Union definiert sind ([EU User Consent Policy](https://www.google.com/about/company/user-consent-policy/)). Die Durchsetzung dieser Änderungen an den Einverständnisanforderungen ist ab dem 6. März 2024 aktiv.
><br/>
...
><br/>
>Kunden, die Adobe Privacy &amp; Security Shield nicht erworben haben, müssen die [Segmentdefinition](../../../segmentation/home.md#segment-definitions)-Funktionen in [Segment Builder](../../../segmentation/ui/segment-builder.md) verwenden, um nicht einverstandene Profile herauszufiltern, damit die bestehenden [!DNL Real-Time CDP] Google-Ziele ohne Unterbrechung weiter verwendet werden können.

[[!DNL Google Customer Match]](https://support.google.com/google-ads/answer/6379332?hl=en) verwendet Ihre Online- und Offline-Daten, um Ihre Kunden über die von Google verwalteten und betriebenen Eigenschaften wie [!DNL Search], [!DNL Shopping] und [!DNL Gmail] zu erreichen und erneut mit ihnen zu interagieren.
...
Damit Sie besser verstehen können, wie und wann das [!DNL Google Customer Match]-Ziel verwendet wird, finden Sie hier einige Beispielanwendungsfälle, die [!DNL Adobe Experience Platform] Kundinnen und Kunden mit dieser Funktion bewältigen können.
...
Je nach Typ der IDs, die Sie in [!DNL Adobe Experience Platform] aufnehmen, müssen Sie die entsprechenden Anforderungen erfüllen.
...
Sie können E-Mail-Adressen hashen, bevor Sie sie in [!DNL Adobe Experience Platform] aufnehmen, oder E-Mail-Adressen in Clear in Experience Platform verwenden und bei der Aktivierung hashen [!DNL Experience Platform].
...
>[!IMPORTANT]
>
> * Die **[!UICONTROL Combine with PII]** Marketing-Aktion ist für das [!DNL Google Customer Match]-Ziel standardmäßig ausgewählt und kann nicht entfernt werden.

### Authentifizierung und Berechtigungen {#authentication-permissions}

Wenn Sie Ihr Google Ads-Konto verbinden, werden Sie von Google aufgefordert, Zugriff auf das Programm von Adobe zu gewähren. Sie müssen die Berechtigung für die Google Ads-API genehmigen, damit Adobe Ihre Kundenlisten erstellen und verwalten kann. Verwenden Sie einen Google Ads-Benutzer mit Standardzugriff oder höherem Zugriff für das Kundenkonto, das Sie aktivieren möchten. Wenn Sie ein Manager-Konto (MCC) verwenden, melden Sie sich mit einem Benutzer im Kundenkonto an und geben Sie die Kundenkonto-ID (nicht die MCC-ID) an.

Wenn die Berechtigung für Google Ads während des OAuth-Flusses nicht gewährt wird, können Aktivierungen später mit Fehlern von der Google Ads-API fehlschlagen. Weitere Informationen [&#x200B; Beheben von berechtigungsbezogenen Fehlern finden &#x200B;](#troubleshooting) im Abschnitt „Fehlerbehebung“.
...
Im **[!UICONTROL Segment schedule]** Schritt müssen Sie die [!UICONTROL App ID] angeben, wenn Sie [!DNL IDFA] oder [!DNL GAID] Zielgruppen an [!DNL Google Customer Match] senden.
...
Weitere Informationen zum Auffinden der [!DNL App ID] finden Sie in der [offiziellen Google-Dokumentation &#x200B;](https://developers.google.com/adwords/api/docs/reference/v201809/AdwordsUserListService.CrmBasedUserList#appid) wenden Sie sich an Ihren Google-Support-Mitarbeiter.
...
* Wählen Sie den `Phone_SHA256_E.164`-Namespace als Quellidentität aus, wenn Sie Telefonnummern bei der Datenaufnahme in [!DNL Experience Platform] gemäß [!DNL Google Customer Match] Anforderungen zum Hashing [&#x200B; Telefonnummern &#x200B;](#phone-number-hashing-requirements).
...
Nachdem Sie eine Verbindung zum Ziel hergestellt und einen Ziel-Datenfluss eingerichtet haben, können Sie die [Überwachungsfunktion](/help/dataflows/ui/monitor-destinations.md) in [!DNL Real-Time CDP] verwenden, um ausführliche Informationen über die Profildatensätze zu erhalten, die für Ihr Ziel in jeder Datenflussausführung aktiviert wurden.
...
### 500 Interner Server-Fehler - Unzureichende Authentifizierungsbereiche {#insufficient-scopes}

Beim Aktivieren von Zielgruppen für dieses Ziel erhalten Sie möglicherweise den folgenden Fehler:
...
Dieser Fehler tritt auf, wenn das für diese Zielverbindung verwendete Google OAuth-Token ohne den erforderlichen Google Ads-API-Bereich erstellt wurde oder wenn der angemeldete Benutzer keine ausreichenden Berechtigungen für das Zielkundenkonto hat.

Gehen Sie wie folgt vor, um dieses Problem zu beheben:

1. **Generieren Sie die Google-** für dieses Zielkonto und stellen Sie sicher, dass Sie die angeforderten Google Ads-Berechtigungen akzeptieren:
   * Navigieren Sie in Experience Platform zu **[!UICONTROL Destinations]** > **[!UICONTROL Accounts]**
   * Finden Sie Ihr Google Customer Match-Konto
   * Wählen Sie **[!UICONTROL More actions]** (⋯) > **[!UICONTROL Edit]** > **[!UICONTROL Renew]**
   * Schließen Sie den Google-Anmelde- und Einverständnisvorgang ab und genehmigen Sie alle angeforderten Berechtigungen
2. Wenn Sie Anzeigen über ein Manager-Konto (MCC) verwalten, bestätigen Sie, dass Sie sich mit einem Benutzer authentifizieren, der [!DNL Standard] oder höher Zugriff auf das Zielkundenkonto hat, und dass die im Ziel konfigurierte **[!UICONTROL Account ID]** die Kundenkonto-ID ist (nicht die MCC-ID).
3. Führen Sie die Aktivierung erneut aus.

Wenn das Problem weiterhin besteht:

* Überprüfen Sie, ob Ihr Google Ads-Konto für den Kundenabgleich auf die Zulassungsliste gesetzt ist und die [Richtlinienanforderungen](#google-account-prerequisites) erfüllt.
* Stellen Sie sicher, dass die Zugriffsebene des Benutzers im Google Ads-Kundenkonto [!DNL Standard] oder höher ist. Weitere Informationen finden Sie in der Dokumentation zu {[}Google Ads .](https://support.google.com/google-ads/answer/9978556?visit_id=637611563637058259-4176462731&rd=1)