...
In den folgenden Abschnitten finden Sie alle Voraussetzungen, die Sie in Experience Platform einrichten müssen, und [!DNL Mailchimp] und für Informationen, die Sie vor der Arbeit mit dem [!DNL Mailchimp Interest Categories] Ziel.
...
Sie benötigen [!DNL Mailchimp] **API-Schlüssel** zum Authentifizieren der [!DNL Mailchimp Interest Categories] Ziel gegen [!DNL Mailchimp] -Konto. Die **API-Schlüssel** dient als **Passwort** wenn Sie [das Ziel authentifizieren](#authenticate).
...
| **[!UICONTROL Zielgruppenname (Bitte wählen Sie zuerst Data Center aus)]** | Nachdem Sie die **[!UICONTROL Rechenzentrum]** enthalten, wird dieses Dropdown-Menü automatisch mit den Zielgruppennamen aus Ihren [!DNL Mailchimp] -Konto. Wählen Sie die Zielgruppe aus, die mit Daten aus Platform aktualisiert werden soll. |
...
So senden Sie Ihre Zielgruppendaten ordnungsgemäß von Adobe Experience Platform an [!DNL Mailchimp Interest Categories] Ziel, müssen Sie den Schritt zur Feldzuordnung durchlaufen. Die Zuordnung besteht darin, eine Verknüpfung zwischen den Schemafeldern Ihres Experience-Datenmodells (XDM) in Ihrem Platform-Konto und den jeweiligen Entsprechungen vom Ziel zu erstellen.
