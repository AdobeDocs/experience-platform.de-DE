...
description: Verwenden Sie das (API) Oracle Eloqua -Ziel, um Ihre Kontodaten zu exportieren und in Oracle Eloqua für Ihre Geschäftsanforderungen zu aktivieren.
...
>[!NOTE]
>
>* [!DNL Oracle Eloqua] benutzerdefinierten Kontaktfelder werden automatisch mit den Namen der Zielgruppen erstellt, die im **[!UICONTROL Select segments]** Schritt ausgewählt wurden.
...
Suchen Sie in **[!UICONTROL Destinations]** > **[!UICONTROL Catalog]** nach [!DNL (API) Oracle Eloqua]. Alternativ können Sie es unter der Kategorie **[!UICONTROL Email Marketing]** finden.
...
1. Wählen Sie im Fenster **[!UICONTROL Select target field]** die Option **[!UICONTROL Select identity namespace]** und wählen Sie eine Identität aus, oder wählen Sie **[!UICONTROL Select custom attributes]** aus und geben Sie den gewünschten Attributnamen in das Feld **[!UICONTROL Attribute name]** ein. Der von Ihnen angegebene Attributname sollte mit einem vorhandenen Kontaktattribut in [!DNL Oracle Eloqua] übereinstimmen. Unter [[!DNL create a contact]](https://docs.oracle.com/en/cloud/saas/marketing/eloqua-rest-api/op-api-rest-1.0-data-contact-post.html) finden Sie die genauen Attributnamen, die Sie in [!DNL Oracle Eloqua] verwenden können.
...
>[!IMPORTANT]
>
>* In der **[!UICONTROL Target field]** angegebene Attribute sollten genau wie in der [[!DNL Create a contact]](https://docs.oracle.com/en/cloud/saas/marketing/eloqua-rest-api/op-api-rest-1.0-data-contact-post.html) angegeben benannt werden, da diese Attribute den Anfragetext bilden.
>* Die in der **[!UICONTROL Source field]** angegebenen Attribute entsprechen keiner dieser Einschränkungen. Sie können sie nach Bedarf zuordnen. Wenn das Datenformat jedoch beim Pushen in [!DNL Oracle Eloqua] nicht korrekt ist, führt dies zu einem Fehler. Sie können beispielsweise die **[!UICONTROL Source field]** Identity-Namespace-`contact key`, -`ABC ID` usw. zuordnen. nach **[!UICONTROL Target field]** : `EloquaId`, nachdem sichergestellt wurde, dass die ID-Werte mit dem Format übereinstimmen, das von [!DNL Oracle Eloqua] akzeptiert wird.
...
1. Gehen Sie zur [!DNL Oracle Eloqua]-Website und navigieren Sie dann zur Seite **[!UICONTROL Contacts Overview]** , um zu überprüfen, ob die Profile aus der Audience hinzugefügt wurden. Um den Status der Audience anzuzeigen, gehen Sie zu einer **[!UICONTROL Contact Detail]** Seite und überprüfen Sie, ob das Kontaktfeld mit dem ausgewählten Audience-Namen als Präfix erstellt wurde.
...
|---|---|---|
| April 2023 | Dokumentation aktualisieren | <ul><li>Wir haben den Abschnitt [Anwendungsfälle](#use-cases) mit einem klareren Beispiel dafür aktualisiert, wann Kundinnen und Kunden von der Verwendung dieses Ziels profitieren würden.</li> <li>Der Abschnitt [Zuordnung](#mapping-considerations-example) wurde mit klaren Beispielen für obligatorische und optionale Zuordnungen aktualisiert.</li> <li>Der Abschnitt [Mit dem Ziel verbinden](#connect) wurde mit einem Beispiel aktualisiert, in dem beschrieben wird, wie der verkettete Wert für das Feld **[!UICONTROL Username]** mithilfe des [!DNL Oracle Eloqua]-Firmennamens und des [!DNL Oracle Eloqua]-Benutzernamens erstellt wird. (PLATIR-28343)</li><li>Die Abschnitte [Sammeln [!DNL Oracle Eloqua] Anmeldeinformationen](#gather-credentials) und [Ausfüllen der Zieldetails](#destination-details) wurden mit Anleitungen zur [!DNL Oracle Eloqua] **[!UICONTROL Pod]** aktualisiert. Der *Pod“*-Wert wird vom Ziel verwendet, um die Basis-URL für die API-Aufrufe zu erstellen. Der Abschnitt [[!DNL Oracle Eloqua] Voraussetzungen](#prerequisites-destination) wurde auch mit Anleitungen zum Zuweisen von *„Erweiterte Benutzer - Marketing-Berechtigungen“* als erforderliches *„Sicherheitsgruppen“* für Ihre [!DNL Oracle Eloqua]-Instanz aktualisiert.</li></ul> |
