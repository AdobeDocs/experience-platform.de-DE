...
>[!AVAILABILITY]
>
>Diese Funktion steht Kunden zur Verfügung, die das [!DNL Real-Time CDP] Prime- oder Ultimate-Paket, [!DNL Adobe Journey Optimizer] oder Customer Journey Analytics erworben haben. Wenden Sie sich an den Adobe-Support, um weitere Informationen zu erhalten.
...
>
>Informationen [&#x200B; Bearbeiten des Enddatums &#x200B;](#scheduling) Datensatzexport-Datenflusses finden Sie im Abschnitt „Planung“.

In diesem Artikel wird der Workflow erläutert, der zum Exportieren [Datensätze](/help/catalog/datasets/overview.md) von [!DNL Adobe Experience Platform] an Ihren bevorzugten Cloud-Speicherort, z. B. [!DNL Amazon S3], SFTP-Speicherorte oder [!DNL Google Cloud Storage], mithilfe der Experience Platform-Benutzeroberfläche erforderlich ist.
...
Die Datensätze, die Sie exportieren können, variieren je nach Experience Platform-Programm ([!DNL Real-Time CDP], [!DNL Adobe Journey Optimizer]), Ebene (Prime oder Ultimate) und allen Add-ons, die Sie erworben haben (z. B. Data Distiller).
...
<tbody>
  ...
  <tr>
    ...
    <td>Prime</td>
    <td>Weitere Informationen finden Sie in der <a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html?lang=de#datasets"> [!DNL Adobe Journey Optimizer]</a>.</td>
  </tr>
  <tr>
    <td>Ultimate</td>
    <td>Weitere Informationen finden Sie in der <a href="https://experienceleague.adobe.com/docs/journey-optimizer/using/data-management/datasets/export-datasets.html?lang=de#datasets"> [!DNL Adobe Journey Optimizer]</a>.</td>
...
Beachten Sie die folgenden Voraussetzungen zum Exportieren von Datensätzen:
...
1. Wählen Sie **[!UICONTROL Activate]** oder **[!UICONTROL Export datasets]** auf der Karte aus, die dem Ziel entspricht, an das Sie Datensätze exportieren möchten.
...
1. Klicken Sie auf **[!UICONTROL Data type Datasets]** und wählen Sie die Zielverbindung aus, in die Sie Datensätze exportieren möchten, und klicken Sie dann auf **[!UICONTROL Next]**.

>[!TIP]
> 
>Wenn Sie ein neues Ziel zum Exportieren von Datensätzen einrichten möchten, wählen Sie **[!UICONTROL Configure new destination]** aus, um den Workflow [Mit Ziel verbinden](/help/destinations/ui/connect-destination.md) Trigger.
...
1. Die **[!UICONTROL Select datasets]** wird angezeigt. Fahren Sie mit dem nächsten Abschnitt fort, um die [Datensätze auszuwählen](#select-datasets), die exportiert werden sollen.

## Auswählen Ihrer Datensätze {#select-datasets}

Aktivieren Sie die Kontrollkästchen links neben den Datensatznamen, um die Datensätze auszuwählen, die Sie an das Ziel exportieren möchten, und klicken Sie dann auf **[!UICONTROL Next]**.
...
>[!NOTE]
>
>Alle hier ausgewählten Datensätze verwenden denselben Exportzeitplan. Wenn Sie unterschiedliche Exportpläne benötigen (z. B. inkrementelle Exporte für einige Datensätze und einmalige vollständige Exporte für andere), erstellen Sie für jeden Planungstyp separate Datenflüsse.
...
>[!IMPORTANT]
>
>**Zeitplan gilt für alle Datensätze im Datenfluss**
>
>Wenn Sie den Exportzeitplan konfigurieren oder ändern, gilt er für **alle Datensätze** die derzeit über den von Ihnen konfigurierten Datenfluss exportiert werden. Sie können nicht verschiedene Zeitpläne für einzelne Datensätze innerhalb desselben Datenflusses festlegen.
>
>Wenn Sie für verschiedene Datensätze unterschiedliche Exportpläne benötigen, müssen Sie für jeden Planungstyp separate Datenflüsse (separate Zielverbindungen) erstellen.
>
>**Beispiel** Wenn Sie Datensatz A inkrementell exportieren lassen und Datensatz B mit einem einmaligen vollständigen Exportzeitplan hinzufügen, wird Datensatz A auch auf den einmaligen vollständigen Exportzeitplan aktualisiert.

Verwenden Sie den **[!UICONTROL Scheduling]** Schritt, um:
...
>[!WARNING]
>
>Wenn Sie den Zeitplan hier ändern, wird das Exportverhalten für alle Datensätze in diesem Datenfluss aktualisiert. Wenn dieser Datenfluss mehrere Datensätze enthält, sind alle von dieser Änderung betroffen.
...
Die Option **[!UICONTROL Export incremental files]** ist standardmäßig ausgewählt. Dies führt zu einem Trigger einer oder mehrerer Dateien, die einen vollständigen Schnappschuss des Datensatzes darstellen. Nachfolgende Dateien sind inkrementelle Ergänzungen zum Datensatz seit dem vorherigen Export. Sie können auch **[!UICONTROL Export full files]** auswählen. Wählen Sie in diesem Fall die **[!UICONTROL Once]** für einen einmaligen vollständigen Export des Datensatzes aus.
...
1. Verwenden Sie den **[!UICONTROL Frequency]**, um die Exportfrequenz auszuwählen:
   ...
   * **[!UICONTROL Daily]**: Planung von inkrementellen Dateiexporten einmal täglich, jeden Tag, zum angegebenen Zeitpunkt.
   * **[!UICONTROL Hourly]**: Planung von inkrementellen Dateiexporten alle 3, 6, 8 oder 12 Stunden.

2. Verwenden Sie den **[!UICONTROL Time]**, um die Tageszeit [!DNL UTC] Format auszuwählen, zu der der Export erfolgen soll.

3. Verwenden Sie den **[!UICONTROL Date]**-Selektor, um das Intervall auszuwählen, in dem der Export stattfinden soll.

4. Wählen Sie **[!UICONTROL Save]** aus, um den Zeitplan zu speichern, und fahren Sie mit dem **[!UICONTROL Review]** Schritt fort.
...
Wählen Sie **[!UICONTROL Edit folder path]** aus, um die Ordnerstruktur an Ihrem Speicherort anzupassen, an dem exportierte Datensätze abgelegt werden.
...
### Best Practices für die Verwaltung mehrerer Datensätze {#best-practices-multiple-datasets}

Beachten Sie beim Exportieren mehrerer Datensätze die folgenden Best Practices:

* **Anforderungen an denselben Zeitplan**: Gruppieren Sie Datensätze, die denselben Exportzeitplan (Häufigkeit, Typ) benötigen, in einem einzigen Datenfluss, um die Verwaltung zu erleichtern.
* **Unterschiedliche Zeitplananforderungen**: Erstellen Sie separate Datenflüsse für Datensätze, für die unterschiedliche Exportpläne oder Exporttypen erforderlich sind (inkrementell vs. vollständig). Dadurch wird sichergestellt, dass jeder Datensatz entsprechend seinen spezifischen Anforderungen exportiert wird.
* **Vor dem Ändern überprüfen**: Bevor Sie den Zeitplan für einen vorhandenen Datenfluss ändern, überprüfen Sie, welche Datensätze bereits über diesen Datenfluss exportiert werden, um unbeabsichtigte Änderungen an ihrem Exportverhalten zu vermeiden.
* **Dokumentieren der Einrichtung**: Verfolgen Sie, welche Datensätze sich in welchen Datenflüssen befinden, insbesondere bei der Verwaltung mehrerer Exportpläne über verschiedene Ziele hinweg.

## Überprüfung {#review}

Auf der Seite **[!UICONTROL Review]** können Sie eine Zusammenfassung Ihrer Auswahl sehen. Wählen Sie **[!UICONTROL Cancel]** aus, um den Fluss abzubrechen, **[!UICONTROL Back]**, Ihre Einstellungen zu ändern, oder **[!UICONTROL Finish]**, Ihre Auswahl zu bestätigen und mit dem Exportieren von Datensätzen an das Ziel zu beginnen.
...
>[!TIP]
>
>Die erste Ebene in dieser Ordnerstruktur - `folder-name-you-provided` - stellt die **[!UICONTROL Folder path]** dar, die Sie angegeben haben, als Sie [mit dem Ziel verbunden](/help/destinations/ui/connect-destination.md#set-up-connection-parameters) Datensätze exportieren.
...
1. Wechseln Sie zur [Experience Platform](https://experience.adobe.com/platform/)Benutzeroberfläche und wählen Sie **[!UICONTROL Destinations]** über die linke Navigationsleiste aus. Wählen Sie **[!UICONTROL Browse]** in der oberen Kopfzeile aus, um Ihre vorhandenen Ziel-Datenflüsse anzuzeigen.
...
2. Wählen Sie in der Spalte **[!UICONTROL Activation data]** das Steuerelement für Datensätze aus, um alle Datensätze anzuzeigen, die diesem Exportdatenfluss zugeordnet sind.
...
3. Die **[!UICONTROL Activation data]** für das Ziel wird angezeigt. Aktivieren Sie die Kontrollkästchen links in der Datensatzliste, um die Datensätze auszuwählen, die Sie entfernen möchten, und klicken Sie dann in der rechten Leiste auf **[!UICONTROL Remove datasets]** , um den Trigger zum Bestätigungsdialogfeld zum Entfernen des Datensatzes anzuzeigen.
...
4. Wählen Sie im Bestätigungsdialog die Option **[!UICONTROL Remove]** aus, um den Datensatz sofort aus den Exporten in das Ziel zu entfernen.
...
Informationen dazu, wie viele Daten Sie pro Jahr für jede Experience Platform-Anwendung exportieren dürfen, finden Sie in den Produktbeschreibungsdokumenten. Sie können beispielsweise die [!DNL Real-Time CDP] Produktbeschreibung ([) &#x200B;](https://helpx.adobe.com/de/legal/product-descriptions/real-time-customer-data-platform-b2c-edition-prime-and-ultimate-packages.html).

Beachten Sie, dass die Berechtigungen für den Datenexport für verschiedene Programme nicht additiv sind. Wenn Sie beispielsweise [!DNL Real-Time CDP] Ultimate und [!DNL Adobe Journey Optimizer] Ultimate erwerben, ist die Berechtigung für den Profilexport gemäß den Produktbeschreibungen die größere der beiden Berechtigungen. Die Berechnung Ihrer Volumenberechtigungen erfolgt anhand der Gesamtzahl der lizenzierten Profile und der Multiplikation mit 500 KB für [!DNL Real-Time CDP] Prime bzw. 700 KB für [!DNL Real-Time CDP] Ultimate, um zu bestimmen, wie viel Datenvolumen Ihnen zusteht.
...
* Monitoring-Metriken für Datensatzexporte werden derzeit mit Zahlen für Profilexporte gemischt, sodass sie nicht die tatsächlichen Exportzahlen widerspiegeln.
* Daten mit einem Zeitstempel, der älter als 365 Tage ist, werden aus Datensatzexporten ausgeschlossen. Weitere Informationen finden Sie in der [Leitplanken für geplante Datensatzexporte](/help/destinations/guardrails.md#scheduled-dataset-exports)
...
**Kann ich für verschiedene Datensätze im selben Datenfluss unterschiedliche Exportzeitpläne festlegen?**

+++Antwort
Nein, alle Datensätze innerhalb eines einzigen Datenflusses verwenden denselben Exportzeitplan. Wenn Sie für verschiedene Datensätze unterschiedliche Exportpläne benötigen, müssen Sie für jeden Planungstyp separate Datenflüsse (Zielverbindungen) erstellen. Wenn Sie beispielsweise möchten, dass Datensatz A täglich inkrementell exportiert und Datensatz B als einmaligen vollständigen Export exportiert, müssen Sie zwei separate Datenflüsse erstellen.
+++
