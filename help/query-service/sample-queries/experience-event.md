...
title: Beispielabfragen für Erlebnisereignisse
type: Tutorial
...
# Beispielabfragen für [!DNL Experience Events]

Zusätzlich zu Standard-SQL-Abfragen bietet Adobe Experience Platform [!DNL Query Service] unterstützt das Schreiben von Abfragen mithilfe von [!DNL Experience Events]. Erlebnisereignisse werden durch die Experience-Datenmodell (XDM)-ExperienceEvent-Klasse repräsentiert, die einen unveränderlichen und nicht aggregierten Schnappschuss des Systems erfasst, wenn ein Benutzer mit einer Website oder einem Dienst interagiert, und daher für die Zeitbereichsanalyse verwendet werden kann.

Weitere Informationen zu XDM und [!DNL Experience Events] finden Sie im Abschnitt [[!DNL XDM System] Übersicht](../../xdm/home.md). Durch Kombination von [!DNL Query Service] mit [!DNL Experience Events]können Sie Verhaltenstrends unter Ihren Benutzern effektiv verfolgen. Das folgende Dokument enthält Beispiele für Abfragen, die Folgendes beinhalten: [!DNL Experience Events].
...
Die Spalte mit dem Zeitstempel in [!DNL Experience Event] -Datensätze sind in UTC enthalten. Im folgenden Beispiel wird die `from_utc_timestamp()`-Funktion verwendet, um den Zeitstempel von UTC in EDT umzuwandeln. Anschließend wird mit der `date_format()`-Funktion das Datum vom Rest des Zeitstempels getrennt.
...
Weitere Informationen zu Beispielabfragen mit Adobe Defined Functions (ADFs) finden Sie im Handbuch für Adobe Defined Functions. Allgemeine Hinweise zur Ausführung von Abfragen finden Sie im [Handbuch zum Ausführen von Abfragen in Query Service](../best-practices/writing-queries.md).
