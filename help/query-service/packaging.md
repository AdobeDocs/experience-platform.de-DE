...
- **Ad-hoc** Abfragen sind SQL-Abfragen, mit denen aufgenommene Datensätze zur Überprüfung, Validierung, Experimentierung usw. untersucht werden. Diese Abfragen schreiben keine Daten zurück in den Data Lake von Platform.
- **Batch-Abfragen** sind SQL-Abfragen, die zur Nachbearbeitung aufgenommener Datensätze verwendet werden. Diese Abfragen bereinigen, formen, bearbeiten und reichern Daten an, deren Ergebnisse zurück in den Data Lake von Platform geschrieben werden. Diese Abfragen können als Batch-Vorgänge geplant, verwaltet und überwacht werden.

Query Service-Funktionen sind in den folgenden Produkten und Add-ons enthalten:

- **Plattformbasierte Anwendungen** (Adobe Real-time Customer Data Platform, Adobe Customer Journey Analytics und Adobe Journey Optimizer): Der Zugriff auf den Abfrage-Service zum Ausführen von Ad-hoc-Abfragen wird von Anfang an für jede Variante und Ebene von plattformbasierten Anwendungen bereitgestellt.
...
| Berechtigung für Abfrage-Service | Mit Platform-basierten Anwendungen verpackt | Verpackt mit [!DNL Data Distiller] |
|---|---|---|
...
| Data-Lake-Speicherkapazität | Ihre gesamten Speicherberechtigungen hängen von Ihren plattformbasierten Anwendungslizenzen ab. Beispielsweise Real-Time CDP, AJO, CJA usw. | Ja - Es wird eine zusätzliche Speicherberechtigung bereitgestellt, um Ihre Roh- und abgeleiteten Datensätze für Distiller-Anwendungsfälle über ein siebentägiges Datenablaufdatum hinaus beizubehalten.<br>Ihre Data-Lake-Speicherkapazität wird in Terabyte (TB) gemessen und hängt von der Menge der erworbenen Rechenstunden ab. Weitere Informationen finden Sie in der Produktbeschreibung . |
| Freibetrag für Datenexporte | Ihre gesamte Exportberechtigung hängt von Ihren plattformbasierten Anwendungslizenzen ab. Beispielsweise Real-Time CDP, AJO, CJA usw. | Ja - Es wird eine zusätzliche Exportberechtigung bereitgestellt, um den Export abgeleiteter Datensätze zu ermöglichen, die mit Data Distiller erstellt wurden.<br>Ihre jährliche Datenexportzulage wird in Terabyte (TB) gemessen und hängt von der Menge der von Ihnen erworbenen Rechenstunden ab. Weitere Einzelheiten finden Sie in der Produktbeschreibung. |
