...
Um eine HubSpot-Verbindung zu erstellen, muss ein Satz von HubSpot-Verbindungsspezifikationen im Flow Service vorhanden sein. Der erste Schritt beim Verbinden von Platform mit HubSpot besteht darin, diese Spezifikationen abzurufen.

**API-Format**

Jede verfügbare Quelle verfügt über einen eigenen Satz von Verbindungsspezifikationen, um Verbindungseigenschaften wie Authentifizierungsanforderungen zu beschreiben. Beim Senden einer GET-Anforderung an den `/connectionSpecs` Endpunkt werden Verbindungsspezifikationen für alle verfügbaren Quellen zurückgegeben. Sie können auch die Abfrage einschließen, Informationen speziell für HubSpot `property=name=="hubspot"` zu erhalten.
...
Die folgende Anforderung ruft die Verbindungsspezifikationen für HubSpot ab.
...
Eine erfolgreiche Antwort gibt die Verbindungsspezifikation für HubSpot einschließlich der eindeutigen Kennung (`id`) zurück. Diese ID ist im nächsten Schritt erforderlich, um eine Verbindung für die API zu erstellen.
...
Eine Verbindung für die API gibt eine Quelle an und enthält Ihre Anmeldeinformationen für diese Quelle. Pro HubSpot-Konto ist nur eine Verbindung für die API erforderlich, da sie zur Erstellung mehrerer Quell-Connectors verwendet werden kann, um verschiedene Daten einzubringen.
...
In diesem Lernprogramm haben Sie eine HubSpot-Verbindung mit der Flow Service API erstellt und den eindeutigen ID-Wert der Verbindung erhalten. Sie können diese Verbindungs-ID im nächsten Lernprogramm verwenden, um zu erfahren, wie Sie CRM-Systeme mithilfe der Flow Service API [erkunden](../../explore/crm.md).