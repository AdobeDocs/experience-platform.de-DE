...
Das folgende Tutorial führt Sie durch die Schritte zum Erstellen einer Quellverbindung und eines Datenflusses, um [!DNL Mailchimp Members]-Daten mithilfe der [[!DNL Flow Service] -API](https://www.adobe.io/experience-platform-apis/references/flow-service/) an Platform zu übermitteln.
...
Nachdem Sie Ihre [!DNL Mailchimp]-Authentifizierungs-Anmeldedaten abgerufen haben, können Sie jetzt mit dem Erstellen des Datenflusses beginnen, um [!DNL Mailchimp Members]-Daten in Platform zu bringen. Der erste Schritt bei der Erstellung eines Datenflusses besteht darin, eine Basisverbindung zu erstellen.
...
Um eine [!DNL Mailchimp] Basisverbindung mit einfacher Authentifizierung zu erstellen, stellen Sie eine POST-Anfrage an den `/connections`-Endpunkt [!DNL Flow Service] -API und geben dabei die Anmeldeinformationen für Ihre `authorizationTestUrl`, `username` und `password` an.
...
Um eine [!DNL Mailchimp] Basisverbindung mit OAuth 2-Aktualisierungs-Code zu erstellen, stellen Sie eine POST-Anfrage an den `/connections`-Endpunkt, während Sie Anmeldeinformationen für Ihre `authorizationTestUrl` und `accessToken` bereitstellen.
...
>[!TIP]
>
>Um den akzeptierten Formattyp für `{SOURCE_PARAMS}` abzurufen, müssen Sie die gesamte Zeichenfolge `list_id` in base64 kodieren. Beispielsweise entspricht `"list_id": "10c097ca71"` in base64 kodiert `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`.
...
| `params.listId` | Die Listen-ID [!DNL Mailchimp] wird auch als Zielgruppen-ID bezeichnet und ermöglicht die Übertragung von Zielgruppendaten an andere Integrationen. |
