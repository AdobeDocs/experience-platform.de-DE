...
# (Beta) Erstellen Sie einen Datenfluss für eine [!DNL OneTrust Integration] -Quelle, die [!DNL Flow Service] API
...
| `auth.params.` | Enthält die Anmeldeinformationen, die zum Authentifizieren Ihrer Quelle erforderlich sind. Der Root-Host und das Zugriffstoken für die Verbindung mit der API. |
| `auth.params.host` | Die Umgebung, aus der die [!DNL OneTrust Integration] -Daten abgerufen werden. |
