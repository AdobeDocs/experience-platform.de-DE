...
title: Erfassen von Marketing-Automatisierungsdaten mithilfe von Quell-Connectoren und APIs
...
Für dieses Tutorial benötigen Sie Zugriff auf ein Marketing-Automatisierungssystem von Drittanbietern über eine gültige Verbindung und Informationen über die Datei, die Sie in Platform einbringen möchten, einschließlich des Pfads und der Struktur der Datei. Wenn Sie diese Informationen nicht haben, lesen Sie das Tutorial [Erkunden eines Drittanbieter-Marketing-Automatisierungssystems mithilfe der Flow Service-API](../explore/marketing-automation.md) , bevor Sie dieses Tutorial ausführen.
...
Eine erfolgreiche Antwort gibt Details zum neu erstellten Schema zurück, einschließlich der eindeutigen Kennung (`$id`). Speichern Sie diese ID so, wie es in späteren Schritten erforderlich ist, um einen Zieldatensatz, eine Zielzuordnung und einen Datenfluss zu erstellen.
...
Eine erfolgreiche Antwort gibt Details zum neu erstellten Mapping zurück, einschließlich der eindeutigen Kennung (`id`). Speichern Sie diesen Wert, da er in einem späteren Schritt zum Erstellen eines Datenflusses erforderlich ist.
...
In diesem Tutorial haben Sie einen Quell-Connector erstellt, um Daten aus einem Marketing-Automatisierungssystem auf geplanter Basis zu erfassen. Eingehende Daten können jetzt von nachgelagerten Platform-Diensten wie [!DNL Real-time Customer Profile] und [!DNL Data Science Workspace] verwendet werden. Weitere Informationen finden Sie in den folgenden Dokumenten:
