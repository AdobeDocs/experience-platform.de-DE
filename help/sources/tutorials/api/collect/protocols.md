...
# Erstellen eines Datenflusses für Protokollquellen mithilfe der [!DNL Flow Service]-API

In diesem Tutorial werden die Schritte zum Abrufen von Daten aus einer Protokollquelle und zum Übertragen dieser Daten auf Platform mithilfe der [[!DNL Flow Service] -API](https://www.adobe.io/experience-platform-apis/references/flow-service/) behandelt.
...
* [[!DNL Experience Data Model (XDM) System]](../../../../xdm/home.md): Das standardisierte Framework, mit dem in Experience Platform Kundenerlebnisdaten organisiert werden.
   ...
   * [Entwicklerhandbuch zu Schema Registry](../../../../xdm/api/getting-started.md): Enthält wichtige Informationen, die Sie benötigen, um erfolgreich Aufrufe an die Schema Registry-API durchzuführen. Diese umfassen Ihre `{TENANT_ID}`, das Konzept sogenannter „Container“ und die für Anfragen erforderlichen Kopfzeilen, von denen insbesondere die Accept-Kopfzeile und deren mögliche Werte wichtig sind.
...
* [[!DNL Batch ingestion]](../../../../ingestion/batch-ingestion/overview.md): Mit der Batch-Aufnahme-API können Sie Daten als Batch-Dateien in Experience Platform aufnehmen.
...
| -------- | ----------- |
| `baseConnectionId` | Die Verbindungs-ID Ihres Protokollprogramms |
| `params.path` | Der Pfad der Quelldatei. |
| `connectionSpec.id` | Die Verbindungsspezifikations-ID Ihres Protokollprogramms. |
...
Ein Ziel-XDM-Schema kann erstellt werden, indem eine POST-Anfrage an die [Schema Registry-API](https://www.adobe.io/experience-platform-apis/references/schema-registry/) durchgeführt wird.
...
Ein Zieldatensatz kann erstellt werden, indem eine POST-Anfrage, die die ID des Zielschemas in der Payload angibt, an die [Catalog Service API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/catalog.yaml) durchgeführt wird,
...
Sie verfügen jetzt über die eindeutigen Kennungen, ein Zielschema, einen Zieldatensatz und die Verbindungsspezifikations-ID zum Data Lake. Wenn Sie die [!DNL Flow Service]-API verwenden, können Sie eine Zielverbindung erstellen, indem Sie diese Kennungen zusammen mit dem Datensatz angeben, der die eingehenden Quelldaten enthält.
...
Bei einer erfolgreichen Antwort wird die eindeutige Kennung der neuen Zielverbindung (`id`) zurückgegeben. Dieser Wert ist in einem späteren Schritt zum Erstellen eines Datenflusses erforderlich.
...
Um einen Zuordnungssatz zu erstellen, stellen Sie eine POST-Anfrage an den Endpunkt `mappingSets` der [[!DNL Data Prep] -API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/data-prep.yaml) und geben Sie dabei Ihr XDM-Zielschema `$id` und die Details der Zuordnungssätze an, die Sie erstellen möchten.
...
Bei einer erfolgreichen Antwort werden Details zu der neu erstellten Zuordnung zurückgegeben, einschließlich ihrer eindeutigen Kennung (`id`). Diese ID ist in einem späteren Schritt erforderlich, um einen Datenfluss zu erstellen.
...
Ein Datenfluss sorgt für die Erfassung von Daten aus Quellen und deren Aufnahme in Platform. Um einen Datenfluss zu erstellen, müssen Sie zunächst die Datenflussspezifikationen abrufen, die für die Datenerfassung aus Ihrem Protokollprogramm zuständig sind.
...
Bei einer erfolgreichen Antwort werden die Details der Datenflussspezifikation zurückgegeben, die für die Übermittlung von Daten aus Ihrer Quelle an Platform sorgt. Die Antwort enthält die eindeutige Flussspezifikation `id`, die zum Erstellen eines neuen Datenflusses benötigt wird.
...
| `transformations.params.deltaColum` | Die Spalte, die verwendet wird, um zwischen neuen und vorhandenen Daten zu unterscheiden. Inkrementelle Daten werden basierend auf dem Zeitstempel der ausgewählten Spalte aufgenommen. Das unterstützte Format für `deltaColumn` bei Verwendung von generischen OData ist `yyyy-MM-ddTHH:mm:ssZ`. |
