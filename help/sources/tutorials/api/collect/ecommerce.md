...
# Erstellen Sie einen Datenfluss für E-Commerce-Quellen mithilfe der [!DNL Flow Service] API

In diesem Tutorial werden die Schritte zum Abrufen von Daten aus einer E-Commerce-Quelle und zum Übertragen dieser Daten auf Platform mithilfe von [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).

>[!NOTE]
>
>Um einen Datenfluss zu erstellen, müssen Sie bereits über eine gültige [!DNL Shopify] Kennung der Basisverbindung. Wenn Sie diese ID nicht haben, lesen Sie die Schritte zum [Erstellen Sie eine [!DNL Shopify] Basisverbindung](../create/ecommerce/shopify.md) vor dem Versuch, dieses Tutorial durchzuführen.
...
Ein Datenfluss ist für die Erfassung von Daten aus Quellen und deren Aufnahme in [!DNL Platform]. Um einen Datenfluss zu erstellen, müssen Sie zunächst die Datenflussspezifikationen abrufen, indem Sie eine GET-Anfrage an die [!DNL Flow Service] API. Datenflussspezifikationen sind für die Erfassung von Daten aus einer E-Commerce-Quelle verantwortlich.
...
| `scheduleParams.frequency` | Die `frequency` an dem der Datenfluss Daten erfasst. Zulässige Werte sind: `once`, `minute`, `hour`, `day`oder `week`. |
| `scheduleParams.interval` | Das Intervall bezeichnet den Zeitraum zwischen zwei aufeinander folgenden Durchsatzausführungen. Der Wert des Intervalls sollte eine Ganzzahl ungleich null sein. Ein Intervall ist nicht erforderlich, wenn `frequency` festgelegt als `once` und sollte größer oder gleich sein als `15` für andere `frequency` -Werte. |
...
In diesem Tutorial haben Sie einen Quell-Connector erstellt, um Daten-E-Commerce auf geplanter Basis zu erfassen. Eingehende Daten können jetzt von nachgelagerten Systemen verwendet werden [!DNL Platform] Dienste wie [!DNL Real-time Customer Profile] und [!DNL Data Science Workspace]. Weitere Informationen finden Sie in den folgenden Dokumenten:
