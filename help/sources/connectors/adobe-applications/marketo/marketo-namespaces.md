...
| `TECHNICAL_ACCOUNT_ID` | Eine Berechtigung, die zur Integration mit [Adobe Developer Console verwendet ](https://developer.adobe.com/). Gehen Sie wie folgt vor, um Ihre ID des technischen Kontos abzurufen:<ul><li>Navigieren Sie zu [Adobe Developer Console](https://developer.adobe.com/) und öffnen Sie Ihr Projekt.</li><li>Öffnen Sie die **Berechtigung**, die _Service-Konto (JWT)_ verwendet. **Hinweis**: Dies kann in der älteren Benutzeroberfläche als „Enterprise“ bezeichnet werden.</li><li>Die Anmeldeinformationen sollten Ihre Client-ID, ID des technischen Kontos, Organisations-ID und andere JWT-Metadaten enthalten.</li></ul>. | `D42AEVJZTTJC6LZADUBVPA15@techacct.adobe.com` |
...
| `PLATFORM_URL` | Der URL-Endpunkt, an den Sie API-Aufrufe durchführen. Dieser Wert ist fest und immer auf `platform.adobe.io/` festgelegt. | `platform.adobe.io/` |
...
Weitere Informationen zum Schemakompositionsmodell, einschließlich Planungsgrundsätzen und Best Practices, finden Sie in den [Grundlagen der Schemakomposition](../../../../xdm/schema/composition.md).
