...
Mit der Adobe Experience Platform können Sie Adobe Analytics-Daten über den Analytics Data Connector (ADC) erfassen. Einige der über ADC erfassten Daten können direkt aus Analytics-Feldern Erlebnisdatenmodellfeldern (XDM) zugeordnet werden, während andere Daten Transformationen und spezifische Funktionen erfordern, um erfolgreich zugeordnet zu werden.

![](../images/analytics-data-experience-platform.png)

## Direkte Zuordnungsfelder

Auswahlfelder werden direkt von Adobe Analytics dem Experience Data Model (XDM) zugeordnet.
...
| --------------- | --------- | -------- | ---------- |
| m_evar1 - m_evar250 | _experience.analytics.customDimensions.eVars.eVar1 - _experience.analytics.customDimensions.eVars.eVar250 | string | Eine benutzerdefinierte Variable, die zwischen 1 und 250 liegen kann. Jede Organisation verwendet diese benutzerspezifischen eVars unterschiedlich. |
| m_prop1 - m_prop75 | _experience.analytics.customDimensions.props.prop1 - _experience.analytics.customDimensions.props.prop75 | string | Benutzerspezifische Traffic-Variablen, die zwischen 1 und 75 liegen können. |
...
| m_Kampagne | marketing.trackingCode | string | Die in der Dimension &quot;Rückverfolgungscode&quot;verwendete Variable. |
| m_Kanal | web.webPageDetails.siteSection | string | Die in der Dimension &quot;Sitebereiche&quot;verwendete Variable. |
| m_domain | environment.domain | string | Die in der Dimension &quot;Domäne&quot;verwendete Variable. Dies erfolgt auf der Grundlage des Internet-Dienstleisters (ISP) des Benutzers. |
| m_geo_city | placeContext.geo.city | string | Der Name der Stadt des Treffers. Dies basiert auf der IP-Adresse des Treffers. |
...
| m_geo_region | placeContext.geo.stateProvince | string | Der Name des Bundesstaates oder der Region des Treffers. Dies basiert auf der IP-Adresse des Treffers. |
| m_geo_zip | placeContext.geo.postalCode | string | Die Postleitzahl des Treffers. Dies basiert auf der IP-Adresse des Treffers. |
| m_keywords | search.keywords | string | Die in der Suchbegriffdimension verwendete Variable. |
...
| m_page_url | web.webPageDetails.URL | string | Die URL des Seitentreffers. |
| m_pagename_no_url | web.webPageDetails.</span>name | string | Eine Variable, mit der die Dimension &quot;Seiten&quot;gefüllt wird. |
| m_Werber | web.webReferrer.URL | string | Die Seiten-URL der vorherigen Seite. |
...
| m_state | _experience.analytics.customDimensions.stateProvince | string | Without context I&#39;m afraid I can&#39;t assess which version is the correct one. |
| m_user_server | web.webPageDetails.server | string | Eine Variable, die in der Serverdimension verwendet wird. |
| m_zip | _experience.analytics.customDimensions.postalCode | string | Eine Variable, mit der die Dimension &quot;Postleitzahl&quot;gefüllt wird. |
| accept_language | environment.browserDetails.acceptLanguage | string | Liste aller zulässigen Sprachen, wie im Accept-Language HTTP-Header angegeben. |
...
| user_agent | environment.browserDetails.userAgent | string | Die Benutzeragenten-Zeichenfolge, die im HTTP-Header gesendet wird. |
| mobileappid | application.</span>name | string | Die mobile App-ID, die im folgenden Format gespeichert wird: `[AppName][BundleVersion]`. |
| mobiledevice | device.model | string | Der Name des Mobilgeräts. Unter iOS als kommagetrennte 2-Ziffern-Zeichenfolge gespeichert. Die erste Zahl steht für die Gerätegenerierung und die zweite Zahl für die Gerätefamilie. |
| pointofinterest | placeContext.POIinteraction.POIDetail.</span>name | string | Wird von Mobildiensten verwendet. Stellt den Interessenbereich dar. |
| pointofinterestdistance | placeContext.POIinteraction.POIDetail.geoInteractionDetails.distanceToCenter | Anzahl | Wird von Mobildiensten verwendet. Stellt den Zielpunkt-Abstand dar. |
| mobileplaceaccuracy | placeContext.POIinteraction.POIDetail.geoInteractionDetails.deviceGeoAccuracy | Anzahl | Erfasst mit der Kontextdatenvariablen a.loc.acc. Gibt die Genauigkeit des GPS in Metern zum Erfassungszeitpunkt an. |
...
| mobileplaceid | placeContext.POIinteraction.POIDetail.POIID | string | Wird aus der Kontextdatenvariablen a.loc.id erfasst. Kennung für einen bestimmten Zielpunkt. |
| video | media.mediaTimed.primaryAssetReference._id | string | Der Name des Videos. |
| videoad | advertising.adAssetReference._id | string | Bezeichner des Anzeigenassets. |
| videocontenttype | media.mediaTimed.primaryAssetViewDetails.broadcastContentType | string | Der Videoinhalt-Typ. Für alle Video-Ansichten ist dies automatisch auf &quot;Video&quot;eingestellt. |
| videoadpod | advertising.adAssetViewDetails.adBreak._id | string | Der Pod, in dem sich die Videoanzeige befindet. |
| videoadinpod | advertising.adAssetViewDetails.index | Ganzzahl | Die Position der Videoanzeige im Pod. |
| videoplayername | media.mediaTimed.primaryAssetViewDetails.playerName | string | Der Name des Videoplayers. |
| videochannel | media.mediaTimed.primaryAssetViewDetails.broadcastChannel | string | Der Video-Kanal. |
| videoadplayername | advertising.adAssetViewDetails.playerName | string | Der Name des Videoanzeigenplayers. |
| videochapter | media.mediaTimed.mediaChapter.chapterAssetReference._id | string | Name des Kapitels &quot;Video&quot; |
| videoname | media.mediaTimed.primaryAssetReference._dc.title | string | Der Videoname. |
| videoadname | advertising.adAssetReference._dc.title | string | Der Name der Videoanzeige. |
| videoshow | media.mediaTimed.primaryAssetReference._iptc4xmpExt.Series._iptc4xmpExt.Name | string | Videosendung. |
| videoseason | media.mediaTimed.primaryAssetReference._iptc4xmpExt.Season._iptc4xmpExt.Name | string | Videosaison. |
| videoepisode | media.mediaTimed.primaryAssetReference._iptc4xmpExt.Episode._iptc4xmpExt.Name | string | Videoepisode. |
| videonetwork | media.mediaTimed.primaryAssetViewDetails.broadcastNetwork | string | Videonetzwerk. |
| videoshowtype | media.mediaTimed.primaryAssetReference.showType | string | Typ der Videosendung. |
| videoadload | media.mediaTimed.primaryAssetViewDetails.adLoadType | string | Ladeaufforderungen der Videoanzeige. |
| videofeedtype | media.mediaTimed.primaryAssetViewDetails.sourceFeed | string | Video-Feed-Typ. |
| mobilebeaconmajor | placeContext.POIinteraction.POIDetail.beaconInteractionDetails.beaconMajor | Anzahl | Mobile Services – Haupt-Beacon. |
| mobilebeaconminor | placeContext.POIinteraction.POIDetail.beaconInteractionDetails.beaconMinor | Anzahl | Mobile Services – Neben-Beacon. |
...
| videosessionid | media.mediaTimed.primaryAssetViewDetails._id | string | Videositzungs-ID. |
...
| videoqoebuffercount | media.mediaTimed.primaryAssetViewDetails.qoe.buffers | Objekt | Videoqualität – Anzahl Puffer | {id (Zeichenfolge), Wert (Zahl)} |
...
| videoqoebitratechangecount | media.mediaTimed.primaryAssetViewDetails.qoe.bitrateChanges | Objekt | Videoqualität – Anzahl Änderungen | {id (Zeichenfolge), Wert (Zahl)} |
| videoqoebitrateaverage | media.mediaTimed.primaryAssetViewDetails.qoe.bitrateAverage | Objekt | Videoqualität – Durchschnittliche Bitrate | {id (Zeichenfolge), Wert (Zahl)} |
| videoqoeerrorcount | media.mediaTimed.primaryAssetViewDetails.qoe.errors | Objekt | Videoqualität – Anzahl Fehler | {id (Zeichenfolge), Wert (Zahl)} |
...
| mobileosversion | Umgebung.operatingSystem, Umgebung.operatingSystemVersion | string | Version des mobilen Betriebssystems. |
| videoadlength | advertising.adAssetReference._xmpDM.duration | Ganzzahl | Länge der Videoanzeige |

## Generierte Zuordnungsfelder

Auswahlfelder, die von ADC stammen, müssen transformiert werden, sodass eine Logik über eine direkte Kopie von Adobe Analytics hinaus erforderlich ist, um in XDM generiert zu werden.
...
| m_geo_country | placeContext.geo.countryCode | string | Abkürzung des Landes, aus dem der Treffer stammt, das auf dem UZ basiert. |
| m_geo_latitude | placeContext.geo._Schema.latitude | Anzahl | <!-- MISSING --> |
| m_geo_Längengrad | placeContext.geo._Schema.Längengrad | Anzahl | <!-- MISSING --> |
| m_java_enabled | environment.browserDetails.javaEnabled | Boolescher Wert | Eine Flag, die angibt, ob Java aktiviert ist. |
| m_latitude | placeContext.geo._Schema.latitude | Anzahl | <!-- MISSING --> |
| m_Längengrad | placeContext.geo._Schema.Längengrad | Anzahl | <!-- MISSING --> |
| m_page_Ereignis_var1 | web.webInteraction.URL | string | Eine Variable, die nur zur Linktracking von Bildanforderungen verwendet wird. Diese Variable enthält die URL des angeklickten Downloadlinks, Exitlinks oder benutzerspezifischen Links. |
| m_page_Ereignis_var2 | web.webInteraction.name | string | Eine Variable, die nur zur Linktracking von Bildanforderungen verwendet wird. Auf diese Weise wird der benutzerdefinierte Name des Links Liste, sofern er angegeben ist. |
...
| m_pagename_no_url | web.webPageDetails.pageViews.value | Anzahl | Der Name der Seite (falls festgelegt). Wenn keine Seite angegeben ist, bleibt dieser Wert leer. |
...
| m_search_engine | search.searchEngine | string | Die numerische ID, die die Suchmaschine darstellt, die den Besucher auf Ihre Site verwiesen hat. |
| post_currency | commerce.order.currencyCode | string | Der während der Transaktion verwendete Währungscode. |
| post_cust_hit_time_gmt | timestamp | string | Dies wird nur in für Zeitstempel aktivierten Datensätzen verwendet. Dieser Zeitstempel wird basierend auf der Unix-Zeit mit dem Zeitstempel gesendet. |
...
| post_cust_visid | endUserIDs._experience.aacustomid.Namensraum.code | string | Die Kunden-Besucher-ID. |
| post_visid_high + visid_low | identityMap | object | Eine eindeutige ID für einen Besuch. |
| post_visid_high + visid_low | endUserIDs._experience.aaid.id | string | Eine eindeutige ID für einen Besuch. |
...
| post_visid_high | endUserIDs._experience.aaid.Namensraum.code | string | Wird in Verbindung mit visid_low verwendet, um einen Besuch eindeutig zu identifizieren. |
...
| hit_time_gmt | receiveTimestamp | string | Der Zeitstempel des Treffers, basierend auf der Unix-Zeit. |
| hitid_high + hitid_low | _id | string | Eine eindeutige Kennung zur Identifizierung eines Treffers. |
| hitid_low | _id | string | Wird in Verbindung mit hitid_high verwendet, um einen Treffer eindeutig zu identifizieren. |
| ip | environment.ipV4 | string | Die IP-Adresse, basierend auf dem HTTP-Header der Bildanforderung. |
...
| mcvisid_high + mcvisid_low | endUserIDs._experience.mcid.id | string | Die Experience Cloud-Besucher-ID. |
| mcvisid_high | endUserIDs._experience.mcid.primary | Boolescher Wert | Die Experience Cloud-Besucher-ID. |
| mcvisid_high | endUserIDs._experience.mcid.Namensraum.code | string | Die Experience Cloud-Besucher-ID. |
| mcvisid_low | identityMap | object | Die Experience Cloud-Besucher-ID. |
| sdid_high + sdid_low | _experience.Zielgruppe.additionalDataID | string | Trefferstitching-ID. Das Analysefeld sdid_high und sdid_low ist die zusätzliche Daten-ID, mit der zwei (oder mehr) eingehende Treffer zusammengeführt werden. |
...
| Analytics-Feld | XDM-Feld | XDM-Typ | Beschreibung |
| --------------- | --------- | -------- | ---------- |
| post_evar1 - post_evar250 | _experience.analytics.customDimensions.eVars.eVar1 - _experience.analytics.customDimensions.eVars.eVar250 | string | Eine benutzerdefinierte Variable, die zwischen 1 und 250 liegen kann. Jede Organisation verwendet diese benutzerspezifischen eVars unterschiedlich. |
| post_prop1 - post_prop75 | _experience.analytics.customDimensions.props.prop1 - _experience.analytics.customDimensions.props.prop75 | string | Benutzerspezifische Traffic-Variablen, die zwischen 1 und 75 liegen können. |
...
| post_Kampagne | marketing.trackingCode | string | Die in der Dimension &quot;Rückverfolgungscode&quot;verwendete Variable. |
| post_Kanal | web.webPageDetails.siteSection | string | Die in der Dimension &quot;Sitebereiche&quot;verwendete Variable. |
| post_cust_visid | endUserIDs._experience.aacustomid.id | string | Die benutzerdefinierte Besucher-ID, falls festgelegt. |
| post_first_hit_page_url | _experience.analytics.endUser.firstWeb.webPageDetails.URL | string | Die URL der ersten Seite, die der Besucher erreicht. |
| post_first_hit_pagename | _experience.analytics.endUser.firstWeb.webPageDetails.name | string | Eine Variable, die in der Dimension &quot;Ursprüngliche Entrypage&quot;verwendet wird. Der Seitenname der Entrypage des Besuchers. |
| post_keywords | search.keywords | string | Die Suchbegriffe, die für den Treffer erfasst wurden. |
| post_page_url | web.webPageDetails.URL | string | Die URL des Seitentreffers. |
| post_pagename_no_url | web.webPageDetails.name | string | Eine Variable, mit der die Dimension &quot;Seiten&quot;gefüllt wird. |
| post_purchaseid | commerce.order.purchaseID | string | Variable, die zur eindeutigen Identifizierung von Käufen verwendet wird. |
| post_Werber | web.webReferrer.URL | string | Die URL der vorherigen Seite. |
| post_state | _experience.analytics.customDimensions.stateProvince | string | Without context I&#39;m afraid I can&#39;t assess which version is the correct one. |
| post_user_server | web.webPageDetails.server | string | Eine Variable, die in der Serverdimension verwendet wird. |
| post_zip | _experience.analytics.customDimensions.postalCode | string | Eine Variable, mit der die Dimension &quot;Postleitzahl&quot;gefüllt wird. |
| browser | _experience.analytics.Umgebung.browserID | Ganzzahl | Die numerische ID des Browsers. |
| domain | environment.domain | string | Die in der Dimension &quot;Domäne&quot;verwendete Variable. Dies erfolgt auf der Grundlage des Internet-Dienstleisters (ISP) des Benutzers. |
| first_hit_Werber | _experience.analytics.endUser.firstWeb.webReferrer.URL | string | Die erste verweisende URL für den Besucher. |
| geo_city | placeContext.geo.city | string | Der Name der Stadt des Treffers. Dies basiert auf der IP-Adresse des Treffers. |
...
| geo_region | placeContext.geo.stateProvince | string | Der Name des Bundesstaates oder der Region des Treffers. Dies basiert auf der IP-Adresse des Treffers. |
| geo_zip | placeContext.geo.postalCode | string | Die Postleitzahl des Treffers. Dies basiert auf der IP-Adresse des Treffers. |
...
| visit_keywords | _experience.analytics.session.search.keywords | string | Eine Variable, die in der Dimension &quot;Suchbegriffe&quot;verwendet wird. |
...
| visit_Werber | _experience.analytics.session.web.webReferrer.URL | string | Die erste verweisende Stelle des Besuchs. |
...
| post_latitude | placeContext.geo._Schema.latitude | Anzahl | <!-- MISSING --> |
| post_Längengrad | placeContext.geo._Schema.Längengrad | Anzahl | <!-- MISSING --> |
| post_page_Ereignis | web.webInteraction.type | string | Der Typ des Treffers, der in der Bildanforderung gesendet wird (Standard-Treffer, Downloadlink, Ausstiegslink oder benutzerspezifischer Link angeklickt). |
| post_page_Ereignis | web.webInteraction.linkClicks.value | Anzahl | Der Typ des Treffers, der in der Bildanforderung gesendet wird (Standard-Treffer, Downloadlink, Ausstiegslink oder benutzerspezifischer Link angeklickt). |
| post_page_Ereignis_var1 | web.webInteraction.URL | string | Diese Variable wird nur bei der Linktracking-Bildanforderungen verwendet. Dies ist die URL des angeklickten Downloadlinks, Exitlinks oder benutzerspezifischen Links. |
| post_page_Ereignis_var2 | web.webInteraction.name | string | Diese Variable wird nur bei der Linktracking-Bildanforderungen verwendet. Dies ist der benutzerdefinierte Name des Links. |
...
| post_pagename_no_url | web.webPageDetails.pageViews.value | Anzahl | Der Name der Seite (falls festgelegt). Wenn keine Seite angegeben ist, bleibt dieser Wert leer. |
...
| post_search_engine | search.searchEngine | string | Die numerische ID, die die Suchmaschine darstellt, die den Besucher auf Ihre Site verwiesen hat. |
...
| first_hit_ref_type | _experience.analytics.endUser.firstWeb.webReferrer.type | string | Die numerische ID, die den Werber-Typ des ersten Werbers des Besuchers darstellt. |
...
| geo_country | placeContext.geo.countryCode | string | Abkürzung für das Land, aus dem der Treffer stammt, basierend auf der IP. |
| geo_latitude | placeContext.geo._Schema.latitude | Anzahl | <!-- MISSING --> |
| geo_Längengrad | placeContext.geo._Schema.Längengrad | Anzahl | <!-- MISSING --> |
...
| visit_ref_type | _experience.analytics.session.web.webReferrer.type | string | Numerische ID des Typs der verweisenden Stelle der ersten verweisenden Stelle des Besuchs. |
| visit_search_engine | _experience.analytics.session.search.searchEngine | string | Numerische ID der ersten Suchmaschine des Besuchs. |
